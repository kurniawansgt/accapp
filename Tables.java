 ...
 *    -  <Table>s objs = new <Table>s(0); // All Data
 *    or 
 *    - <Table>s obj = new <Table>s(<Table>.PROPERTY_<field>, <value>);
 *      Sample :
 *      <Table>s obj = new <Table>s(<Table>.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            <Table>s brs = new <Table>s(0);

            for (Iterator<<Table>> it = brs.iterator(); it.hasNext(); ) {
                
                <Table> br = (<Table>) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNama<Table>());
 ...
 * @author windu
 */
public class <Table>s  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "<Table>s";
    public static final String SQLNAME = <Table>.TABLE;
    public static String DEFAULTSORT = <Table>.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = <Table>.PROPERTY_ID;
    ...
    private String[][] column = {
        {<Table>.PRIMARYKEY, <Table>.PRIMARYKEY, "200"}, 
        {<Table>.PRIMARYKEY, <Table>.PRIMARYKEY, "200"} 
    };
    ...
    public <Table>s() throws CodeException {
        this (new <Table>(NULLID));
        _isLoaded_ = true;
    }
    
    public <Table>s(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new <Table>(ID);
        ID_ = ID;
    }
    
    public <Table>s(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new <Table>(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public <Table>s(<Table> parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public <Table>s(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public <Table>s(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(<Table> obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(<Table>s obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public void setParent(<Table> obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return <Table>s.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return <Table>s.SQLNAME;
    }
