 ...
 *    -  Masterproduks objs = new Masterproduks(0); // All Data
 *    or 
 *    - Masterproduks obj = new Masterproduks(Masterproduk.PROPERTY_<field>, <value>);
 *      Sample :
 *      Masterproduks obj = new Masterproduks(Masterproduk.PROPERTY_ID, "1");
 *      It Means : Select * from Masterproduk where id = "1";
 *    - Resultset
 *      Sample :
 * 
 *      DbBean db = DbBean.connect();
        String sql = "SELECT * FROM Masterproduk Where id = 1" ;
 * 
        try {
 ...
 * 2. Read Data 
        try {
            ...
            Masterproduks brs = new Masterproduks(0);

            for (Iterator<Masterproduk> it = brs.iterator(); it.hasNext(); ) {
                
                Masterproduk br = (Masterproduk) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterproduk());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Masterproduk br = (Masterproduk) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterproduk());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Masterproduk br = (Masterproduk) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterproduk());
            }
 ...
 * @author windu
 */
public class Masterproduks  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Masterproduks";
    public static final String SQLNAME = Masterproduk.TABLE;
    public static String DEFAULTSORT = Masterproduk.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Masterproduk.PROPERTY_ID;
    ...
    List<Object> list;
    private Masterproduk parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Masterproduk.PRIMARYKEY, Masterproduk.PRIMARYKEY, "200"}, 
        {Masterproduk.PROPERTY_NAMAPRODUK, Masterproduk.PROPERTY_NAMAPRODUK, "200"} 
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Masterproduks() throws CodeException {
        this (new Masterproduk(NULLID));
        _isLoaded_ = true;
    }
    
    public Masterproduks(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Masterproduk(ID);
        ID_ = ID;
    }
    
    public Masterproduks(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Masterproduk(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Masterproduks(Masterproduk parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Masterproduks(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Masterproduks(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Masterproduk obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Masterproduks obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Masterproduk) it.next());
        }        
    ...
    public Masterproduk getParent() {
        return parentObj_;
    }
    
    public void setParent(Masterproduk obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Masterproduk obj = (Masterproduk) it.next();
    ...
    public String getDefaultSort() {
        return Masterproduks.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Masterproduks.SQLNAME;
    }
