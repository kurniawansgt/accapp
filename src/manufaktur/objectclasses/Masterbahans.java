 ...
 *    -  Masterbahans objs = new Masterbahans(0); // All Data
 *    or 
 *    - Masterbahans obj = new Masterbahans(Masterbahan.PROPERTY_<field>, <value>);
 *      Sample :
 *      Masterbahans obj = new Masterbahans(Masterbahan.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Masterbahans brs = new Masterbahans(0);

            for (Iterator<Masterbahan> it = brs.iterator(); it.hasNext(); ) {
                
                Masterbahan br = (Masterbahan) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterbahan());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Masterbahan br = (Masterbahan) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterbahan());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Masterbahan br = (Masterbahan) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterbahan());
            }
 ...
 * @author windu
 */
public class Masterbahans  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Masterbahans";
    public static final String SQLNAME = Masterbahan.TABLE;
    public static String DEFAULTSORT = Masterbahan.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Masterbahan.PROPERTY_ID;
    ...
    List<Object> list;
    private Masterbahan parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Masterbahan.PRIMARYKEY, Masterbahan.PRIMARYKEY, "200"}, 
        {Masterbahan.PROPERTY_NAMABAHAN, Masterbahan.PROPERTY_NAMABAHAN, "200"} 
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Masterbahans() throws CodeException {
        this (new Masterbahan(NULLID));
        _isLoaded_ = true;
    }
    
    public Masterbahans(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Masterbahan(ID);
        ID_ = ID;
    }
    
    public Masterbahans(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Masterbahan(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Masterbahans(Masterbahan parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Masterbahans(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Masterbahans(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Masterbahan obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Masterbahans obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Masterbahan) it.next());
        }        
    ...
    public Masterbahan getParent() {
        return parentObj_;
    }
    
    public void setParent(Masterbahan obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Masterbahan obj = (Masterbahan) it.next();
    ...
    public String getDefaultSort() {
        return Masterbahans.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Masterbahans.SQLNAME;
    }
