 ...
 * Sample : 
 *    
 * import accapp.component.ap.adjustment.FrmAr_adjbatchlist;
 * 
 * .......
 * 
 * 
 * FrmAr_adjbatchlist frmAr_adjbatchlist;
 * 
 * .......
 * 
 try {
        if (frmAr_adjbatchlist == null) {
            frmmAr_adjbatchlist = new FrmAr_adjbatchlist(this);
        }
        
        showForm(frmAr_adjbatchlist);
...
import accapp.component.browse.FrmBrowseAr_invoblAdjustmentByCustomer;
...
import accapp.objectclasses.ar.adjustment.Ar_adjd;
import accapp.objectclasses.ar.adjustment.Ar_adjds;
import accapp.objectclasses.ar.adjustment.Ar_adjh;
...
public class FrmAr_adjbatchlist extends javax.swing.JInternalFrame implements NavigatorFormInt {
    ...
    public Ar_adjbatchlist obj = new Ar_adjbatchlist();

    Ar_adjh adjh = new Ar_adjh();
    Ar_adjds apadjds = new Ar_adjds();
    ...
    public FrmAr_adjbatchlist() throws CodeException {
        initComponents();
    }
    
    public FrmAr_adjbatchlist(FrmMainFrame mf) throws CodeException{
        this.mf = mf;
        initComponents();
    ...
    public void setAr_adjbatchlist(Ar_adjbatchlist obj){
        this.obj=obj;
        try {
            showData();
    ...
    private void initComponents() {
        ...
        jPanel3.add(jlblyop);
        jlblyop.setBounds(470, 80, 40, 13);
        ...
        jlbljrnldebit.setFont(new java.awt.Font("Dialog", 0, 10));
        ...
        jtxtDocType.setBounds(580, 110, 150, 23);
...
private void jtbnNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jtbnNewActionPerformed
    try{
        ...
        adjh.setmop(jcbMOP.getSelectedIndex()+1);
        adjh.setyop(GlobalUtils.toInt(jcbYOP.getSelectedItem().toString()));
   ...
   int idelete = JOptionPane.showConfirmDialog(null, "Do you want to delete entry# " + jtxtdocentry.getText(), "Delete", JOptionPane.YES_NO_OPTION);
    ...
    if (idelete == JOptionPane.YES_OPTION) {
        try{
            ...
            obj=new Ar_adjbatchlist(obj.getID());
            showData();
...
private void jbtnMoveFirstActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnMoveFirstActionPerformed
    ...
    try{
        ...
        if (pos > 0){
            ...
            adjh = (Ar_adjh) obj.getDetail().list().get(0);
            showDataApAdjh();
        }
...
private void jbtnMovePreviousActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnMovePreviousActionPerformed
    ...
    try{
        ...
        if (pos > 0){
            ...
            adjh = (Ar_adjh) obj.getDetail().list().get(pos - 1);
            showDataApAdjh();
...
private void jbtnMoveLastActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnMoveLastActionPerformed
    ...
    try{
        ...
        if (pos < obj.getDetail().list().size()){
            ...
            adjh = (Ar_adjh) obj.getDetail().list().get(obj.getDetail().list().size() - 1);
            showDataApAdjh();
...
private void jbtnBrowseVendorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnBrowseVendorActionPerformed
    ...
    try {
        ...
        Ar_customer arcust=new Ar_customer(jtxtvendcode.getText());
...
private void jbtnBrowseDocNoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnBrowseDocNoActionPerformed
    ...
    if (jtxtvendcode.getText().equals("")) {
        JOptionPane.showMessageDialog(null, "Customer Tolong Diisi");
    }else{
        ...
        try {
           ...
           brs.searchDataAdjust(jtxtvendcode.getText());
            if (brs.size() != 0 ) {
                FrmBrowseAr_invoblAdjustmentByCustomer bd = new FrmBrowseAr_invoblAdjustmentByCustomer(brs);
                bd.custmrcode = jtxtvendcode.getText();
                //bd.doctype = "4";
                bd.retrieveData();
        ...
        switch (col){
            case 1:
                try {
                    ...
                    Ar_distcode glacc = new Ar_distcode((tmodeldetail.getValueAt(row, 1)+"").replace("-", ""));
                    tmodeldetail.setValueAt(glacc.getaccrevcodefmt(), row, 2);
...
private void jtblAdjustmentPropertyChange(java.beans.PropertyChangeEvent evt) {//GEN-FIRST:event_jtblAdjustmentPropertyChange
    ...
    try{
        ...
        if (col == 1){
            Ar_distcode apdist = new Ar_distcode((tmodeldetail.getValueAt(row, 0)+"").replace("-", ""));
            tmodeldetail.setValueAt(apdist.getaccrevcodefmt(), row, 2);
        }
...
private void jtxtvendcodeKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jtxtvendcodeKeyPressed
    ...
    if (evt.getKeyCode()==evt.VK_ENTER){
         Ar_customer arcust;
            try {
                arcust = new Ar_customer(jtxtvendcode.getText());
                jtxtvendcode.setText(arcust.getcustmrcode());
                jtxtVendDescription.setText(arcust.getcustmrname());
                jtxtcurnccode.setText(arcust.getcurnccode());
            } catch (CodeException ex) {
                Logger.getLogger(FrmAr_adjbatchlist.class.getName()).log(Level.SEVERE, null, ex);
            }
...
private void jbtnBrowseActionPerformed(java.awt.event.ActionEvent evt) {                                           
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    new FrmAr_adjbatchlist().setVisible(true);
                } catch (CodeException ex) {
                    ex.printStackTrace();
                }
            }
...
//        lockedData(!jtxtbatchsts.getText().equals("2"));
    ...
    public void searchData() throws CodeException {
        obj = new Ar_adjbatchlist(this.jtxtbatchno.getText());
        mf.setIcon(CLASSNAME);
        showData();
//        lockedData(!jtxtbatchsts.getText().equals("2"));
    ...
    public void showData() throws CodeException {
        setCursor(GlobalUtils.HOURGLASSCURSOR);
             ...
             jtxtid.setText(obj.toString(Ar_adjbatchlist.PROPERTY_ID));
             jtxtbatchno.setText(obj.toString(Ar_adjbatchlist.PROPERTY_BATCHNO));
             try {
                kdatebatchdate.setDate(GlobalUtils.formatDate(obj.toString(Ar_adjbatchlist.PROPERTY_BATCHDATE), "MM/dd/yyyy"));
             }catch(Exception e) {
                kdatebatchdate.setDate(new Date());
             }

             jtxtbatchdesc.setText(obj.toString(Ar_adjbatchlist.PROPERTY_BATCHDESC));
             try{
             ...
             jtxtentrydate.setText(obj.toString(Ar_adjbatchlist.PROPERTY_ENTRYDATE));
             jtxtauditdate.setText(obj.toString(Ar_adjbatchlist.PROPERTY_AUDITDATE));
             jtxtaudituser.setText(obj.toString(Ar_adjbatchlist.PROPERTY_AUDITUSER));
             jtxtcmpnyid.setText(obj.toString(Ar_adjbatchlist.PROPERTY_CMPNYID));
             ...
             if (obj.getDetail().list().isEmpty()) {
                ...
                adjh = (Ar_adjh) obj.getDetail().list().get(obj.getDetail().size()-1);
                setAPAdjhEnable(true);
              }
    ...
    public void setData() throws CodeException {
          ...
          obj.fromString(Ar_adjbatchlist.PROPERTY_ID, jtxtid.getText()) ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_BATCHNO, jtxtbatchno.getText()) ;

          
          obj.fromString(Ar_adjbatchlist.PROPERTY_BATCHDATE,  GlobalUtils.formatDate(kdatebatchdate.getDate(),"yyyy-MM-dd")) ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_BATCHDESC, jtxtbatchdesc.getText()) ;

          obj.fromString(Ar_adjbatchlist.PROPERTY_READYTPOST, jchkReadyToPost.isSelected() == true ? "1" : "0") ;

          obj.fromString(Ar_adjbatchlist.PROPERTY_BATCHTYPE, jtxtbatchtype.getText()) ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_BATCHSTS, jtxtbatchsts.getText()) ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_TOTPAYAMT, adjh.getDetailAmt()+"") ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_PAYENTRCNT, jtxtpayentrcnt.getText().replace(",", "")) ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_ENTRYDATE, jtxtentrydate.getText()) ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_AUDITDATE, jtxtauditdate.getText()) ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_AUDITUSER, jtxtaudituser.getText()) ;
          obj.fromString(Ar_adjbatchlist.PROPERTY_CMPNYID, jtxtcmpnyid.getText()) ;
...
private void setDataApAdjh() throws CodeException {
          ...
          adjh.fromString(Ar_adjh.PROPERTY_BATCHNO, jtxtbatchno.getText()) ;
          adjh.fromString(Ar_adjh.PROPERTY_DOCENTRY, jtxtdocentry.getText()) ;
          adjh.fromString(Ar_adjh.PROPERTY_CUSTMRCODE, jtxtvendcode.getText()) ;
          adjh.fromString(Ar_adjh.PROPERTY_DOCNUM, jtxtdocnum.getText()) ;
          adjh.fromString(Ar_adjh.PROPERTY_YOP, jcbYOP.getSelectedItem()+"") ;
          adjh.fromString(Ar_adjh.PROPERTY_MOP, jcbMOP.getSelectedItem()+"") ;
          adjh.fromString(Ar_adjh.PROPERTY_DSCRIPTION, jtxtdscription.getText()) ;

          adjh.fromString(Ar_adjh.PROPERTY_DOCDATE, GlobalUtils.formatDate(kdateentrydate1.getDate(),"yyyy-MM-dd"));

          adjh.fromString(Ar_adjh.PROPERTY_TXTDESC, jtxttxtdesc.getText()) ;
          adjh.fromString(Ar_adjh.PROPERTY_GLREFF, jtxtglreff.getText()) ;
          adjh.fromString(Ar_adjh.PROPERTY_ADJNO, jtxtadjno.getText()) ;
          adjh.fromString(Ar_adjh.PROPERTY_PAYENTCNT, jtxtpayentcnt.getText().replace(",", "")) ;
          
          adjh.fromString(Ar_adjh.PROPERTY_DOCTOTALPAYAMT, jtxtdoctotalpayamt.getText().replace(",", "")) ;

          adjh.fromString(Ar_adjh.PROPERTY_ADJUSTTOT, jtxtadjusttot.getText().replace(",", "")) ;


          adjh.fromString(Ar_adjh.PROPERTY_DOCTYPE, jtxtDocType.getText()) ;
...
public void showDataApAdjh() throws CodeException  {
    ...
    jtxtvendcode.setText(adjh.toString(Ar_adjh.PROPERTY_CUSTMRCODE));
    Ar_customer arcust=new Ar_customer(adjh.getcustmrcode());
    jtxtVendDescription.setText(arcust.getcustmrname());
    jtxtdocnum.setText(adjh.toString(Ar_adjh.PROPERTY_DOCNUM));
    jtxtdscription.setText(adjh.toString(Ar_adjh.PROPERTY_DSCRIPTION));
    ...
    jtxttxtdesc.setText(adjh.toString(Ar_adjh.PROPERTY_TXTDESC));
    jtxtglreff.setText(adjh.toString(Ar_adjh.PROPERTY_GLREFF));

    jtxtadjno.setText(adjh.toString(Ar_adjh.PROPERTY_ADJNO));

    jtxtpayentcnt.setText(adjh.getpayentcnt() == 0 ? "1" : adjh.getpayentcnt()+"");

    Ar_invobl apinv = new Ar_invobl();
    apinv.searchData(adjh.getcustmrcode(), adjh.getdocnum());
...
private void cekCbhd(int row,int col) throws CodeException {
    ...
    if (row > apadjds.list().size() - 1) {
        ...
        apajd = (Ar_adjd) apadjds.list().get(row);
    }
