 ...
 *    -  Op_phonetypes objs = new Op_phonetypes(0); // All Data
 *    or 
 *    - Op_phonetypes obj = new Op_phonetypes(Op_phonetype.PROPERTY_<field>, <value>);
 *      Sample :
 *      Op_phonetypes obj = new Op_phonetypes(Op_phonetype.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            for (Iterator<Op_phonetype> it = brs.iterator(); it.hasNext(); ) {
                
                Op_phonetype br = (Op_phonetype) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_phonetype());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Op_phonetype br = (Op_phonetype) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_phonetype());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Op_phonetype br = (Op_phonetype) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_phonetype());
            }
 ...
 * @author windu
 */
public class Op_phonetypes  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Op_phonetypes";
    public static final String SQLNAME = Op_phonetype.TABLE;
    public static String DEFAULTSORT = Op_phonetype.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Op_phonetype.PROPERTY_ID;
    ...
    List<Object> list;
    private Op_phonetype parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Op_phonetype.PRIMARYKEY, Op_phonetype.PRIMARYKEY, "200"}, 
        {Op_phonetype.PROPERTY_DSCRIPTION, Op_phonetype.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Op_phonetypes() throws CodeException {
        this (new Op_phonetype(NULLID));
        _isLoaded_ = true;
    }
    
    public Op_phonetypes(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_phonetype(ID);
        ID_ = ID;
    }
    
    public Op_phonetypes(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_phonetype(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Op_phonetypes(Op_phonetype parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Op_phonetypes(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Op_phonetypes(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Op_phonetype obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Op_phonetypes obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Op_phonetype) it.next());
        }        
    ...
    public Op_phonetype getParent() {
        return parentObj_;
    }
    
    public void setParent(Op_phonetype obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Op_phonetype obj = (Op_phonetype) it.next();
    ...
    public String getDefaultSort() {
        return Op_phonetypes.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Op_phonetypes.SQLNAME;
    }
