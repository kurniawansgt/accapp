 ...
 *    -  Op_paymentterms objs = new Op_paymentterms(0); // All Data
 *    or 
 *    - Op_paymentterms obj = new Op_paymentterms(Op_paymentterm.PROPERTY_<field>, <value>);
 *      Sample :
 *      Op_paymentterms obj = new Op_paymentterms(Op_paymentterm.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Op_paymentterms brs = new Op_paymentterms(0);

            for (Iterator<Op_paymentterm> it = brs.iterator(); it.hasNext(); ) {
                
                Op_paymentterm br = (Op_paymentterm) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_paymentterm());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Op_paymentterm br = (Op_paymentterm) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_paymentterm());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Op_paymentterm br = (Op_paymentterm) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_paymentterm());
            }
 ...
 * @author windu
 */
public class Op_paymentterms  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Op_paymentterms";
    public static final String SQLNAME = Op_paymentterm.TABLE;
    public static String DEFAULTSORT = Op_paymentterm.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Op_paymentterm.PROPERTY_ID;
    ...
    List<Object> list;
    private Op_paymentterm parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Op_paymentterm.PROPERTY_TERMCODE, Op_paymentterm.PROPERTY_TERMCODE, "200"},
        {Op_paymentterm.PROPERTY_DSCRIPTION, Op_paymentterm.PROPERTY_DSCRIPTION, "200"},
        {Op_paymentterm.PROPERTY_DUEDAYS, Op_paymentterm.PROPERTY_DUEDAYS, "100"},
    ...
    public Op_paymentterms() throws CodeException {
        this (new Op_paymentterm(NULLID));
        _isLoaded_ = true;
    }
    
    public Op_paymentterms(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_paymentterm(ID);
        ID_ = ID;
    }
    
    public Op_paymentterms(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_paymentterm(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Op_paymentterms(Op_paymentterm parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Op_paymentterms(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Op_paymentterms(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Op_paymentterm(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Op_paymentterm obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Op_paymentterms obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Op_paymentterm) it.next());
        }        
    ...
    public Op_paymentterm getParent() {
        return parentObj_;
    }
    
    public void setParent(Op_paymentterm obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Op_paymentterm obj = (Op_paymentterm) it.next();
    ...
    public String getDefaultSort() {
        return Op_paymentterms.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Op_paymentterms.SQLNAME;
    }
