 ...
 *    -  Op_addrtypes objs = new Op_addrtypes(0); // All Data
 *    or 
 *    - Op_addrtypes obj = new Op_addrtypes(Op_addrtype.PROPERTY_<field>, <value>);
 *      Sample :
 *      Op_addrtypes obj = new Op_addrtypes(Op_addrtype.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            for (Iterator<Op_addrtype> it = brs.iterator(); it.hasNext(); ) {
                
                Op_addrtype br = (Op_addrtype) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_addrtype());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Op_addrtype br = (Op_addrtype) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_addrtype());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Op_addrtype br = (Op_addrtype) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_addrtype());
            }
 ...
 * @author windu
 */
public class Op_addrtypes  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Op_addrtypes";
    public static final String SQLNAME = Op_addrtype.TABLE;
    public static String DEFAULTSORT = Op_addrtype.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Op_addrtype.PROPERTY_ID;
    ...
    List<Object> list;
    private Op_addrtype parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Op_addrtype.PROPERTY_ADDRTYPECODE, Op_addrtype.PROPERTY_ADDRTYPECODE, "200"},
        {Op_addrtype.PROPERTY_DSCRIPTION, Op_addrtype.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Op_addrtypes() throws CodeException {
        this (new Op_addrtype(NULLID));
        _isLoaded_ = true;
    }
    
    public Op_addrtypes(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_addrtype(ID);
        ID_ = ID;
    }
    
    public Op_addrtypes(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_addrtype(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Op_addrtypes(Op_addrtype parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Op_addrtypes(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Op_addrtypes(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Op_addrtype obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Op_addrtypes obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Op_addrtype) it.next());
        }        
    ...
    public Op_addrtype getParent() {
        return parentObj_;
    }
    
    public void setParent(Op_addrtype obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Op_addrtype obj = (Op_addrtype) it.next();
    ...
    public String getDefaultSort() {
        return Op_addrtypes.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Op_addrtypes.SQLNAME;
    }
