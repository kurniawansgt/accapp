 ...
 *    -  Op_taxs objs = new Op_taxs(0); // All Data
 *    or 
 *    - Op_taxs obj = new Op_taxs(Op_tax.PROPERTY_<field>, <value>);
 *      Sample :
 *      Op_taxs obj = new Op_taxs(Op_tax.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            for (Iterator<Op_tax> it = brs.iterator(); it.hasNext(); ) {
                
                Op_tax br = (Op_tax) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_tax());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Op_tax br = (Op_tax) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_tax());
            }
 ...
 * @author windu
 */
public class Op_taxs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Op_taxs";
    public static final String SQLNAME = Op_tax.TABLE;
    public static String DEFAULTSORT = Op_tax.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Op_tax.PROPERTY_ID;
    ...
    private String[][] column = {
        {Op_tax.PRIMARYKEY, Op_tax.PRIMARYKEY, "200"}, 
        {Op_tax.PROPERTY_DSCRIPTION, Op_tax.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Op_taxs() throws CodeException {
        this (new Op_tax(NULLID));
        _isLoaded_ = true;
    }
    
    public Op_taxs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_tax(ID);
        ID_ = ID;
    }
    
    public Op_taxs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_tax(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Op_taxs(Op_tax parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Op_taxs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Op_taxs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Op_tax obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Op_taxs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Op_tax getParent() {
        return parentObj_;
    }
    
    public void setParent(Op_tax obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Op_taxs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Op_taxs.SQLNAME;
    }
