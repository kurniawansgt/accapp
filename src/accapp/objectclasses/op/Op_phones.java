 ...
 *    -  Op_phones objs = new Op_phones(0); // All Data
 *    or 
 *    - Op_phones obj = new Op_phones(Op_phone.PROPERTY_<field>, <value>);
 *      Sample :
 *      Op_phones obj = new Op_phones(Op_phone.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Op_phones brs = new Op_phones(0);

            for (Iterator<Op_phone> it = brs.iterator(); it.hasNext(); ) {
                
                Op_phone br = (Op_phone) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_phone());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Op_phone br = (Op_phone) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaOp_phone());
            }
 ...
 * @author windu
 */
public class Op_phones  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Op_phones";
    public static final String SQLNAME = Op_phone.TABLE;
    public static String DEFAULTSORT = Op_phone.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Op_phone.PROPERTY_ID;
    ...
    private String[][] column = {
        {Op_phone.PRIMARYKEY, Op_phone.PRIMARYKEY, "200"}, 
        {Op_phone.PRIMARYKEY, Op_phone.PRIMARYKEY, "200"} 
    };
    ...
    public Op_phones() throws CodeException {
        this (new Op_phone(NULLID));
        _isLoaded_ = true;
    }
    
    public Op_phones(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_phone(ID);
        ID_ = ID;
    }
    
    public Op_phones(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Op_phone(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Op_phones(Op_phone parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Op_phones(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Op_phones(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Op_phone obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Op_phones obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Op_phone) it.next());
        }        
    ...
    public Op_phone getParent() {
        return parentObj_;
    }
    
    public void setParent(Op_phone obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Op_phones.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Op_phones.SQLNAME;
    }

    public void searchData(String ownercode, String addrcode, String addrtypecode) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();

        String query = " select * from op_phone "
