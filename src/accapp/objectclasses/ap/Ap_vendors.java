 ...
 *    -  Ap_vendors objs = new Ap_vendors(0); // All Data
 *    or 
 *    - Ap_vendors obj = new Ap_vendors(Ap_vendor.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_vendors obj = new Ap_vendors(Ap_vendor.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_vendors brs = new Ap_vendors(0);

            for (Iterator<Ap_vendor> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_vendor br = (Ap_vendor) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_vendor());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_vendor br = (Ap_vendor) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_vendor());
            }
 ...
 * @author windu
 */
public class Ap_vendors  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_vendors";
    public static final String SQLNAME = Ap_vendor.TABLE;
    public static String DEFAULTSORT = Ap_vendor.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_vendor.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_vendor.PRIMARYKEY, Ap_vendor.PRIMARYKEY, "200"}, 
        {Ap_vendor.PROPERTY_VENDNAME, Ap_vendor.PROPERTY_VENDNAME, "400"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ap_vendors() throws CodeException {
        this (new Ap_vendor(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_vendors(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_vendor(ID);
        ID_ = ID;
    }
    
    public Ap_vendors(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_vendor(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_vendors(Ap_vendor parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_vendors(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_vendors(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_vendor obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_vendors obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ap_vendor) it.next());
        }        
    ...
    public Ap_vendor getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_vendor obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
                Ap_vendor obj = (Ap_vendor) it.next();
    ...
    public String getDefaultSort() {
        return Ap_vendors.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_vendors.SQLNAME;
    }
