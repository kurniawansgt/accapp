 ...
 *    -  Ap_paydmiscs objs = new Ap_paydmiscs(0); // All Data
 *    or 
 *    - Ap_paydmiscs obj = new Ap_paydmiscs(Ap_paydmisc.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_paydmiscs obj = new Ap_paydmiscs(Ap_paydmisc.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_paydmiscs brs = new Ap_paydmiscs(0);

            for (Iterator<Ap_paydmisc> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_paydmisc br = (Ap_paydmisc) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_paydmisc());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_paydmisc br = (Ap_paydmisc) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_paydmisc());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ap_paydmisc br = (Ap_paydmisc) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_paydmisc());
            }
 ...
 * @author windu
 */
public class Ap_paydmiscs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_paydmiscs";
    public static final String SQLNAME = Ap_paydmisc.TABLE;
    public static String DEFAULTSORT = Ap_paydmisc.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_paydmisc.PROPERTY_ID;
    ...
    List<Object> list;
    private Ap_paydmisc parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ap_paydmisc.PRIMARYKEY, Ap_paydmisc.PRIMARYKEY, "200"}, 
        {Ap_paydmisc.PRIMARYKEY, Ap_paydmisc.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_paydmiscs() throws CodeException {
        this (new Ap_paydmisc(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_paydmiscs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_paydmisc(ID);
        ID_ = ID;
    }
    
    public Ap_paydmiscs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_paydmisc(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_paydmiscs(Ap_paydmisc parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_paydmiscs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_paydmiscs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_paydmisc obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_paydmiscs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ap_paydmisc) it.next());
        }        
    ...
    public Ap_paydmisc getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_paydmisc obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ap_paydmisc obj = (Ap_paydmisc) it.next();
    ...
    public String getDefaultSort() {
        return Ap_paydmiscs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_paydmiscs.SQLNAME;
    }


    public void searchData(double batchno, double docentry) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();

        String query = " select * from ap_paydmisc where batchno = " + batchno + "  and docentry = " + docentry;
