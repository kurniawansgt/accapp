 ...
 *    -  Ap_distseths objs = new Ap_distseths(0); // All Data
 *    or 
 *    - Ap_distseths obj = new Ap_distseths(Ap_distseth.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_distseths obj = new Ap_distseths(Ap_distseth.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_distseths brs = new Ap_distseths(0);

            for (Iterator<Ap_distseth> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_distseth br = (Ap_distseth) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_distseth());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_distseth br = (Ap_distseth) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_distseth());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ap_distseth br = (Ap_distseth) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_distseth());
            }
 ...
 * @author windu
 */
public class Ap_distseths  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_distseths";
    public static final String SQLNAME = Ap_distseth.TABLE;
    public static String DEFAULTSORT = Ap_distseth.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_distseth.PROPERTY_ID;
    ...
    List<Object> list;
    private Ap_distseth parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ap_distseth.PRIMARYKEY, Ap_distseth.PRIMARYKEY, "200"}, 
        {Ap_distseth.PROPERTY_DSCRIPTION, Ap_distseth.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ap_distseths() throws CodeException {
        this (new Ap_distseth(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_distseths(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_distseth(ID);
        ID_ = ID;
    }
    
    public Ap_distseths(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_distseth(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_distseths(Ap_distseth parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_distseths(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_distseths(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_distseth obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_distseths obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ap_distseth) it.next());
        }        
    ...
    public Ap_distseth getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_distseth obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
                Ap_distseth obj = (Ap_distseth) it.next();
    ...
    public String getDefaultSort() {
        return Ap_distseths.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_distseths.SQLNAME;
    }
