 ...
 *    -  Ap_remitaddresss objs = new Ap_remitaddresss(0); // All Data
 *    or 
 *    - Ap_remitaddresss obj = new Ap_remitaddresss(Ap_remitaddress.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_remitaddresss obj = new Ap_remitaddresss(Ap_remitaddress.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_remitaddresss brs = new Ap_remitaddresss(0);

            for (Iterator<Ap_remitaddress> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_remitaddress br = (Ap_remitaddress) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_remitaddress());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_remitaddress br = (Ap_remitaddress) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_remitaddress());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ap_remitaddress br = (Ap_remitaddress) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_remitaddress());
            }
 ...
 * @author windu
 */
public class Ap_remitaddresss  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_remitaddresss";
    public static final String SQLNAME = Ap_remitaddress.TABLE;
    public static String DEFAULTSORT = Ap_remitaddress.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_remitaddress.PROPERTY_ID;
    ...
    List<Object> list;
    private Ap_remitaddress parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ap_remitaddress.PRIMARYKEY, Ap_remitaddress.PRIMARYKEY, "200"}, 
        {Ap_remitaddress.PRIMARYKEY, Ap_remitaddress.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_remitaddresss() throws CodeException {
        this (new Ap_remitaddress(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_remitaddresss(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_remitaddress(ID);
        ID_ = ID;
    }
    
    public Ap_remitaddresss(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_remitaddress(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_remitaddresss(Ap_remitaddress parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_remitaddresss(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_remitaddresss(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Ap_remitaddress(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Ap_remitaddress obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_remitaddresss obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ap_remitaddress) it.next());
        }        
    ...
    public Ap_remitaddress getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_remitaddress obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ap_remitaddress obj = (Ap_remitaddress) it.next();
    ...
    public String getDefaultSort() {
        return Ap_remitaddresss.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_remitaddresss.SQLNAME;
    }
