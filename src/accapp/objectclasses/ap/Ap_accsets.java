 ...
 *    -  Ap_accsets objs = new Ap_accsets(0); // All Data
 *    or 
 *    - Ap_accsets obj = new Ap_accsets(Ap_accset.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_accsets obj = new Ap_accsets(Ap_accset.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_accsets brs = new Ap_accsets(0);

            for (Iterator<Ap_accset> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_accset br = (Ap_accset) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_accset());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_accset br = (Ap_accset) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_accset());
            }
 ...
 * @author windu
 */
public class Ap_accsets  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_accsets";
    public static final String SQLNAME = Ap_accset.TABLE;
    public static String DEFAULTSORT = Ap_accset.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_accset.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_accset.PRIMARYKEY, Ap_accset.PRIMARYKEY, "200"}, 
        {Ap_accset.PROPERTY_DSCRIPTION, Ap_accset.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ap_accsets() throws CodeException {
        this (new Ap_accset(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_accsets(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_accset(ID);
        ID_ = ID;
    }
    
    public Ap_accsets(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_accset(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_accsets(Ap_accset parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_accsets(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_accsets(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_accset obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_accsets obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ap_accset) it.next());
        }        
    ...
    public Ap_accset getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_accset obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
                Ap_accset obj = (Ap_accset) it.next();
    ...
    public String getDefaultSort() {
        return Ap_accsets.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_accsets.SQLNAME;
    }
