 ...
 *    -  Ap_invobss objs = new Ap_invobss(0); // All Data
 *    or 
 *    - Ap_invobss obj = new Ap_invobss(Ap_invobs.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_invobss obj = new Ap_invobss(Ap_invobs.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_invobss brs = new Ap_invobss(0);

            for (Iterator<Ap_invobs> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_invobs br = (Ap_invobs) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_invobs());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_invobs br = (Ap_invobs) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_invobs());
            }
 ...
 * @author windu
 */
public class Ap_invobss  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_invobss";
    public static final String SQLNAME = Ap_invobs.TABLE;
    public static String DEFAULTSORT = Ap_invobs.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_invobs.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_invobs.PRIMARYKEY, Ap_invobs.PRIMARYKEY, "200"}, 
        {Ap_invobs.PRIMARYKEY, Ap_invobs.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_invobss() throws CodeException {
        this (new Ap_invobs(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_invobss(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_invobs(ID);
        ID_ = ID;
    }
    
    public Ap_invobss(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_invobs(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_invobss(Ap_invobs parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_invobss(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_invobss(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_invobs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_invobss obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ap_invobs) it.next());
        }        
    ...
    public Ap_invobs getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_invobs obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
                Ap_invobs obj = (Ap_invobs) it.next();
    ...
    public String getDefaultSort() {
        return Ap_invobss.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_invobss.SQLNAME;
    }
