 ...
 *    -  Ap_adjds objs = new Ap_adjds(0); // All Data
 *    or 
 *    - Ap_adjds obj = new Ap_adjds(Ap_adjd.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_adjds obj = new Ap_adjds(Ap_adjd.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_adjds brs = new Ap_adjds(0);

            for (Iterator<Ap_adjd> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_adjd br = (Ap_adjd) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_adjd());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_adjd br = (Ap_adjd) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_adjd());
            }
 ...
 * @author windu
 */
public class Ap_adjds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_adjds";
    public static final String SQLNAME = Ap_adjd.TABLE;
    public static String DEFAULTSORT = Ap_adjd.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_adjd.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_adjd.PRIMARYKEY, Ap_adjd.PRIMARYKEY, "200"}, 
        {Ap_adjd.PRIMARYKEY, Ap_adjd.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_adjds() throws CodeException {
        this (new Ap_adjd(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_adjds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_adjd(ID);
        ID_ = ID;
    }
    
    public Ap_adjds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_adjd(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_adjds(Ap_adjd parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_adjds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_adjds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_adjd obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_adjds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ap_adjd getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_adjd obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ap_adjds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_adjds.SQLNAME;
    }
    ...
    public void searchData(double btchentry, double journalid) throws CodeException {
        ...
        String query = " select * from ap_adjd where batchno = " + btchentry + "  and docentry = " + journalid;
