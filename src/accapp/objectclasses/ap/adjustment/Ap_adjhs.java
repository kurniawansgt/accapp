 ...
 *    -  Ap_adjhs objs = new Ap_adjhs(0); // All Data
 *    or 
 *    - Ap_adjhs obj = new Ap_adjhs(Ap_adjh.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_adjhs obj = new Ap_adjhs(Ap_adjh.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_adjhs brs = new Ap_adjhs(0);

            for (Iterator<Ap_adjh> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_adjh br = (Ap_adjh) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_adjh());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_adjh br = (Ap_adjh) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_adjh());
            }
 ...
 * @author windu
 */
public class Ap_adjhs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_adjhs";
    public static final String SQLNAME = Ap_adjh.TABLE;
    public static String DEFAULTSORT = Ap_adjh.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_adjh.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_adjh.PRIMARYKEY, Ap_adjh.PRIMARYKEY, "200"}, 
        {Ap_adjh.PRIMARYKEY, Ap_adjh.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_adjhs() throws CodeException {
        this (new Ap_adjh(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_adjhs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_adjh(ID);
        ID_ = ID;
    }
    
    public Ap_adjhs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_adjh(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_adjhs(Ap_adjh parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_adjhs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_adjhs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_adjh obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_adjhs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ap_adjh getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_adjh obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ap_adjhs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_adjhs.SQLNAME;
    }
