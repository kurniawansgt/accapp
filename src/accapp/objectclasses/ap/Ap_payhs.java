 ...
 *    -  Ap_payhs objs = new Ap_payhs(0); // All Data
 *    or 
 *    - Ap_payhs obj = new Ap_payhs(Ap_payh.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_payhs obj = new Ap_payhs(Ap_payh.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_payhs brs = new Ap_payhs(0);

            for (Iterator<Ap_payh> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_payh br = (Ap_payh) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_payh());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_payh br = (Ap_payh) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_payh());
            }
 ...
 * @author windu
 */
public class Ap_payhs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_payhs";
    public static final String SQLNAME = Ap_payh.TABLE;
    public static String DEFAULTSORT = Ap_payh.PROPERTY_DOCENTRY;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_payh.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_payh.PRIMARYKEY, Ap_payh.PRIMARYKEY, "200"}, 
        {Ap_payh.PRIMARYKEY, Ap_payh.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_payhs() throws CodeException {
        this (new Ap_payh(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_payhs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_payh(ID);
        ID_ = ID;
    }
    
    public Ap_payhs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_payh(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_payhs(Ap_payh parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_payhs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_payhs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_payh obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_payhs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ap_payh getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_payh obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ap_payhs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_payhs.SQLNAME;
    }
