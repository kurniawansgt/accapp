 ...
 *    -  Ap_invhs objs = new Ap_invhs(0); // All Data
 *    or 
 *    - Ap_invhs obj = new Ap_invhs(Ap_invh.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_invhs obj = new Ap_invhs(Ap_invh.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_invhs brs = new Ap_invhs(0);

            for (Iterator<Ap_invh> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_invh br = (Ap_invh) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_invh());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_invh br = (Ap_invh) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_invh());
            }
 ...
 * @author windu
 */
public class Ap_invhs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_invhs";
    public static final String SQLNAME = Ap_invh.TABLE;
    public static String DEFAULTSORT = Ap_invh.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_invh.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_invh.PROPERTY_DOCENTRY, Ap_invh.PROPERTY_DOCENTRY, "50"},
        {Ap_invh.PROPERTY_VENDCODE, Ap_invh.PROPERTY_VENDCODE, "100"},
        {Ap_invh.PROPERTY_DOCTYPE, Ap_invh.PROPERTY_DOCTYPE, "50"},
        {Ap_invh.PROPERTY_DOCNUM, Ap_invh.PROPERTY_DOCNUM, "200"},
        {Ap_invh.PROPERTY_DOCTOTALAMT, Ap_invh.PROPERTY_DOCTOTALAMT, "100"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ap_invhs() throws CodeException {
        this (new Ap_invh(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_invhs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_invh(ID);
        ID_ = ID;
    }
    
    public Ap_invhs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_invh(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_invhs(Ap_invh parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_invhs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_invhs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_invh obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_invhs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ap_invh getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_invh obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ap_invhs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_invhs.SQLNAME;
    }
