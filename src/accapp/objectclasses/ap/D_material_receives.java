 ...
 *    -  D_material_receives objs = new D_material_receives(0); // All Data
 *    or 
 *    - D_material_receives obj = new D_material_receives(D_material_receive.PROPERTY_<field>, <value>);
 *      Sample :
 *      D_material_receives obj = new D_material_receives(D_material_receive.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            D_material_receives brs = new D_material_receives(0);

            for (Iterator<D_material_receive> it = brs.iterator(); it.hasNext(); ) {
                
                D_material_receive br = (D_material_receive) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaD_material_receive());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                D_material_receive br = (D_material_receive) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaD_material_receive());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                D_material_receive br = (D_material_receive) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaD_material_receive());
            }
 ...
 * @author windu
 */
public class D_material_receives  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "D_material_receives";
    public static final String SQLNAME = D_material_receive.TABLE;
    public static String DEFAULTSORT = D_material_receive.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = D_material_receive.PROPERTY_ID;
    ...
    List<Object> list;
    private D_material_receive parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {D_material_receive.PRIMARYKEY, D_material_receive.PRIMARYKEY, "200"}, 
        {D_material_receive.PRIMARYKEY, D_material_receive.PRIMARYKEY, "200"} 
    };
    ...
    public D_material_receives() throws CodeException {
        this (new D_material_receive(NULLID));
        _isLoaded_ = true;
    }
    
    public D_material_receives(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new D_material_receive(ID);
        ID_ = ID;
    }
    
    public D_material_receives(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new D_material_receive(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public D_material_receives(D_material_receive parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public D_material_receives(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public D_material_receives(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new D_material_receive(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(D_material_receive obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(D_material_receives obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((D_material_receive) it.next());
        }        
    ...
    public D_material_receive getParent() {
        return parentObj_;
    }
    
    public void setParent(D_material_receive obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                D_material_receive obj = (D_material_receive) it.next();
    ...
    public String getDefaultSort() {
        return D_material_receives.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return D_material_receives.SQLNAME;
    }

    public void searchDataByMR(String nomr) throws CodeException, SQLException {
        String sql = "SELECT * FROM `d_material_receive` WHERE `no_mr`='"+nomr+"'";
