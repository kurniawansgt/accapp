 ...
 *    -  Ap_invds objs = new Ap_invds(0); // All Data
 *    or 
 *    - Ap_invds obj = new Ap_invds(Ap_invd.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_invds obj = new Ap_invds(Ap_invd.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_invds brs = new Ap_invds(0);

            for (Iterator<Ap_invd> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_invd br = (Ap_invd) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_invd());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_invd br = (Ap_invd) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_invd());
            }
 ...
 * @author windu
 */
public class Ap_invds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_invds";
    public static final String SQLNAME = Ap_invd.TABLE;
    public static String DEFAULTSORT = Ap_invd.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_invd.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_invd.PRIMARYKEY, Ap_invd.PRIMARYKEY, "200"}, 
        {Ap_invd.PRIMARYKEY, Ap_invd.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_invds() throws CodeException {
        this (new Ap_invd(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_invds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_invd(ID);
        ID_ = ID;
    }
    
    public Ap_invds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_invd(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_invds(Ap_invd parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_invds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_invds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_invd obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_invds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ap_invd getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_invd obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ap_invds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_invds.SQLNAME;
    }
    ...
    public void searchData(double btchentry, double journalid) throws CodeException {
        ...
        String query = " select * from ap_invd where batchno = " + btchentry + "  and docentry = " + journalid;
