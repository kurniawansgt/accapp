 ...
 *    -  Ap_payds objs = new Ap_payds(0); // All Data
 *    or 
 *    - Ap_payds obj = new Ap_payds(Ap_payd.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_payds obj = new Ap_payds(Ap_payd.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_payds brs = new Ap_payds(0);

            for (Iterator<Ap_payd> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_payd br = (Ap_payd) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_payd());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_payd br = (Ap_payd) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_payd());
            }
 ...
 * @author windu
 */
public class Ap_payds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_payds";
    public static final String SQLNAME = Ap_payd.TABLE;
    public static String DEFAULTSORT = Ap_payd.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_payd.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_payd.PRIMARYKEY, Ap_payd.PRIMARYKEY, "200"}, 
        {Ap_payd.PRIMARYKEY, Ap_payd.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_payds() throws CodeException {
        this (new Ap_payd(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_payds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_payd(ID);
        ID_ = ID;
    }
    
    public Ap_payds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_payd(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_payds(Ap_payd parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_payds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_payds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_payd obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_payds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ap_payd getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_payd obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ap_payds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_payds.SQLNAME;
    }
    ...
    public void searchData(double batchno, double docentry) throws CodeException {
        ...
        String query = " select * from ap_payd where batchno = " + batchno + "  and docentry = " + docentry;
