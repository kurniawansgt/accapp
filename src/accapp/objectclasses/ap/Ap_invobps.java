 ...
 *    -  Ap_invobps objs = new Ap_invobps(0); // All Data
 *    or 
 *    - Ap_invobps obj = new Ap_invobps(Ap_invobp.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_invobps obj = new Ap_invobps(Ap_invobp.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ap_invobps brs = new Ap_invobps(0);

            for (Iterator<Ap_invobp> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_invobp br = (Ap_invobp) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_invobp());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_invobp br = (Ap_invobp) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_invobp());
            }
 ...
 * @author windu
 */
public class Ap_invobps  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_invobps";
    public static final String SQLNAME = Ap_invobp.TABLE;
    public static String DEFAULTSORT = Ap_invobp.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_invobp.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ap_invobp.PRIMARYKEY, Ap_invobp.PRIMARYKEY, "200"}, 
        {Ap_invobp.PRIMARYKEY, Ap_invobp.PRIMARYKEY, "200"} 
    };
    ...
    public Ap_invobps() throws CodeException {
        this (new Ap_invobp(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_invobps(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_invobp(ID);
        ID_ = ID;
    }
    
    public Ap_invobps(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_invobp(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_invobps(Ap_invobp parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_invobps(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_invobps(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_invobp obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_invobps obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ap_invobp) it.next());
        }        
    ...
    public Ap_invobp getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_invobp obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
                Ap_invobp obj = (Ap_invobp) it.next();
    ...
    public String getDefaultSort() {
        return Ap_invobps.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_invobps.SQLNAME;
    }
