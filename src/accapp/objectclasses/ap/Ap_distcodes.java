 ...
 *    -  Ap_distcodes objs = new Ap_distcodes(0); // All Data
 *    or 
 *    - Ap_distcodes obj = new Ap_distcodes(Ap_distcode.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ap_distcodes obj = new Ap_distcodes(Ap_distcode.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            for (Iterator<Ap_distcode> it = brs.iterator(); it.hasNext(); ) {
                
                Ap_distcode br = (Ap_distcode) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_distcode());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ap_distcode br = (Ap_distcode) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_distcode());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ap_distcode br = (Ap_distcode) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAp_distcode());
            }
 ...
 * @author windu
 */
public class Ap_distcodes  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ap_distcodes";
    public static final String SQLNAME = Ap_distcode.TABLE;
    public static String DEFAULTSORT = Ap_distcode.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ap_distcode.PROPERTY_ID;
    ...
    List<Object> list;
    private Ap_distcode parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ap_distcode.PRIMARYKEY, Ap_distcode.PRIMARYKEY, "200"}, 
        {Ap_distcode.PROPERTY_DSCRIPTION, Ap_distcode.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ap_distcodes() throws CodeException {
        this (new Ap_distcode(NULLID));
        _isLoaded_ = true;
    }
    
    public Ap_distcodes(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_distcode(ID);
        ID_ = ID;
    }
    
    public Ap_distcodes(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ap_distcode(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ap_distcodes(Ap_distcode parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ap_distcodes(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ap_distcodes(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ap_distcode obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ap_distcodes obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ap_distcode) it.next());
        }        
    ...
    public Ap_distcode getParent() {
        return parentObj_;
    }
    
    public void setParent(Ap_distcode obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
                Ap_distcode obj = (Ap_distcode) it.next();
    ...
    public String getDefaultSort() {
        return Ap_distcodes.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ap_distcodes.SQLNAME;
    }
