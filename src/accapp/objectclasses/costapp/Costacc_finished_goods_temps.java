 ...
 *    -  Costacc_finished_goods_temps objs = new Costacc_finished_goods_temps(0); // All Data
 *    or 
 *    - Costacc_finished_goods_temps obj = new Costacc_finished_goods_temps(Costacc_finished_goods_temp.PROPERTY_<field>, <value>);
 *      Sample :
 *      Costacc_finished_goods_temps obj = new Costacc_finished_goods_temps(Costacc_finished_goods_temp.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Costacc_finished_goods_temps brs = new Costacc_finished_goods_temps(0);

            for (Iterator<Costacc_finished_goods_temp> it = brs.iterator(); it.hasNext(); ) {
                
                Costacc_finished_goods_temp br = (Costacc_finished_goods_temp) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_finished_goods_temp());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Costacc_finished_goods_temp br = (Costacc_finished_goods_temp) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_finished_goods_temp());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Costacc_finished_goods_temp br = (Costacc_finished_goods_temp) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_finished_goods_temp());
            }
 ...
 * @author windu
 */
public class Costacc_finished_goods_temps  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Costacc_finished_goods_temps";
    public static final String SQLNAME = Costacc_finished_goods_temp.TABLE;
    public static String DEFAULTSORT = Costacc_finished_goods_temp.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Costacc_finished_goods_temp.PROPERTY_ID;
    ...
    List<Object> list;
    private Costacc_finished_goods_temp parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Costacc_finished_goods_temp.PRIMARYKEY, Costacc_finished_goods_temp.PRIMARYKEY, "200"}, 
        {Costacc_finished_goods_temp.PRIMARYKEY, Costacc_finished_goods_temp.PRIMARYKEY, "200"} 
    };
    ...
    public Costacc_finished_goods_temps() throws CodeException {
        this (new Costacc_finished_goods_temp(NULLID));
        _isLoaded_ = true;
    }
    
    public Costacc_finished_goods_temps(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Costacc_finished_goods_temp(ID);
        ID_ = ID;
    }
    
    public Costacc_finished_goods_temps(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Costacc_finished_goods_temp(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Costacc_finished_goods_temps(Costacc_finished_goods_temp parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Costacc_finished_goods_temps(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Costacc_finished_goods_temps(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Costacc_finished_goods_temp(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Costacc_finished_goods_temp obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Costacc_finished_goods_temps obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Costacc_finished_goods_temp) it.next());
        }        
    ...
    public Costacc_finished_goods_temp getParent() {
        return parentObj_;
    }
    
    public void setParent(Costacc_finished_goods_temp obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Costacc_finished_goods_temp obj = (Costacc_finished_goods_temp) it.next();
    ...
    public String getDefaultSort() {
        return Costacc_finished_goods_temps.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Costacc_finished_goods_temps.SQLNAME;
    }

    public void loadData(String bulan, String tahun) throws CodeException {
        DbBean db = DbBean.connect(); 

        String sql = " SELECT * FROM bankjurnal.costacc_finished_goods_temp WHERE mop = " + bulan + " AND yop =  " + tahun ;
