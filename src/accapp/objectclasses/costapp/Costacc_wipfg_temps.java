 ...
 *    -  Costacc_wipfg_temps objs = new Costacc_wipfg_temps(0); // All Data
 *    or 
 *    - Costacc_wipfg_temps obj = new Costacc_wipfg_temps(Costacc_wipfg_temp.PROPERTY_<field>, <value>);
 *      Sample :
 *      Costacc_wipfg_temps obj = new Costacc_wipfg_temps(Costacc_wipfg_temp.PROPERTY_ID, "1");
 *      It Means : Select * from Costacc_wipfg_temp where id = "1";
 *    - Resultset
 *      Sample :
 * 
 *      DbBean db = DbBean.connect();
        String sql = "SELECT * FROM Costacc_wipfg_temp Where id = 1" ;
 * 
        try {
 ...
 * 2. Read Data 
        try {
            ...
            Costacc_wipfg_temps brs = new Costacc_wipfg_temps(0);

            for (Iterator<Costacc_wipfg_temp> it = brs.iterator(); it.hasNext(); ) {
                
                Costacc_wipfg_temp br = (Costacc_wipfg_temp) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_wipfg_temp());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Costacc_wipfg_temp br = (Costacc_wipfg_temp) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_wipfg_temp());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Costacc_wipfg_temp br = (Costacc_wipfg_temp) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_wipfg_temp());
            }
 ...
 * @author windu
 */
public class Costacc_wipfg_temps  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Costacc_wipfg_temps";
    public static final String SQLNAME = Costacc_wipfg_temp.TABLE;
    public static String DEFAULTSORT = Costacc_wipfg_temp.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Costacc_wipfg_temp.PROPERTY_ID;
    ...
    List<Object> list;
    private Costacc_wipfg_temp parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Costacc_wipfg_temp.PRIMARYKEY, Costacc_wipfg_temp.PRIMARYKEY, "200"}, 
        {Costacc_wipfg_temp.PRIMARYKEY, Costacc_wipfg_temp.PRIMARYKEY, "200"} 
    };
    ...
    public Costacc_wipfg_temps() throws CodeException {
        this (new Costacc_wipfg_temp(NULLID));
        _isLoaded_ = true;
    }
    
    public Costacc_wipfg_temps(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Costacc_wipfg_temp(ID);
        ID_ = ID;
    }
    
    public Costacc_wipfg_temps(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Costacc_wipfg_temp(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Costacc_wipfg_temps(Costacc_wipfg_temp parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Costacc_wipfg_temps(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Costacc_wipfg_temps(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Costacc_wipfg_temp(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Costacc_wipfg_temp obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Costacc_wipfg_temps obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Costacc_wipfg_temp) it.next());
        }        
    ...
    public Costacc_wipfg_temp getParent() {
        return parentObj_;
    }
    
    public void setParent(Costacc_wipfg_temp obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Costacc_wipfg_temp obj = (Costacc_wipfg_temp) it.next();
    ...
    public String getDefaultSort() {
        return Costacc_wipfg_temps.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Costacc_wipfg_temps.SQLNAME;
    }

    public void loadData(String bulan, String tahun) throws CodeException {
        DbBean db = DbBean.connect(); 

        String sql = " SELECT * FROM bankjurnal.costacc_wipfg_temp WHERE mop = " + bulan + " AND yop =  " + tahun ;
