 ...
 *    -  Costacc_companys objs = new Costacc_companys(0); // All Data
 *    or 
 *    - Costacc_companys obj = new Costacc_companys(Costacc_company.PROPERTY_<field>, <value>);
 *      Sample :
 *      Costacc_companys obj = new Costacc_companys(Costacc_company.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Costacc_companys brs = new Costacc_companys(0);

            for (Iterator<Costacc_company> it = brs.iterator(); it.hasNext(); ) {
                
                Costacc_company br = (Costacc_company) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_company());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Costacc_company br = (Costacc_company) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_company());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Costacc_company br = (Costacc_company) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCostacc_company());
            }
 ...
 * @author windu
 */
public class Costacc_companys  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Costacc_companys";
    public static final String SQLNAME = Costacc_company.TABLE;
    public static String DEFAULTSORT = Costacc_company.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Costacc_company.PROPERTY_ID;
    ...
    List<Object> list;
    private Costacc_company parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Costacc_company.PRIMARYKEY, Costacc_company.PRIMARYKEY, "200"}, 
        {Costacc_company.PRIMARYKEY, Costacc_company.PRIMARYKEY, "200"} 
    };
    ...
    public Costacc_companys() throws CodeException {
        this (new Costacc_company(NULLID));
        _isLoaded_ = true;
    }
    
    public Costacc_companys(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Costacc_company(ID);
        ID_ = ID;
    }
    
    public Costacc_companys(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Costacc_company(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Costacc_companys(Costacc_company parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Costacc_companys(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Costacc_companys(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Costacc_company(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Costacc_company obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Costacc_companys obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Costacc_company) it.next());
        }        
    ...
    public Costacc_company getParent() {
        return parentObj_;
    }
    
    public void setParent(Costacc_company obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Costacc_company obj = (Costacc_company) it.next();
    ...
    public String getDefaultSort() {
        return Costacc_companys.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Costacc_companys.SQLNAME;
    }
