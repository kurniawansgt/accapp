 ...
 *    -  Ar_rcpdmiscs objs = new Ar_rcpdmiscs(0); // All Data
 *    or 
 *    - Ar_rcpdmiscs obj = new Ar_rcpdmiscs(Ar_rcpdmisc.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_rcpdmiscs obj = new Ar_rcpdmiscs(Ar_rcpdmisc.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_rcpdmiscs brs = new Ar_rcpdmiscs(0);

            for (Iterator<Ar_rcpdmisc> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_rcpdmisc br = (Ar_rcpdmisc) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcpdmisc());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_rcpdmisc br = (Ar_rcpdmisc) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcpdmisc());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ar_rcpdmisc br = (Ar_rcpdmisc) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcpdmisc());
            }
 ...
 * @author windu
 */
public class Ar_rcpdmiscs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_rcpdmiscs";
    public static final String SQLNAME = Ar_rcpdmisc.TABLE;
    public static String DEFAULTSORT = Ar_rcpdmisc.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_rcpdmisc.PROPERTY_ID;
    ...
    List<Object> list;
    private Ar_rcpdmisc parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ar_rcpdmisc.PRIMARYKEY, Ar_rcpdmisc.PRIMARYKEY, "200"}, 
        {Ar_rcpdmisc.PRIMARYKEY, Ar_rcpdmisc.PRIMARYKEY, "200"} 
    };
    ...
    public Ar_rcpdmiscs() throws CodeException {
        this (new Ar_rcpdmisc(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_rcpdmiscs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_rcpdmisc(ID);
        ID_ = ID;
    }
    
    public Ar_rcpdmiscs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_rcpdmisc(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_rcpdmiscs(Ar_rcpdmisc parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_rcpdmiscs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_rcpdmiscs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_rcpdmisc obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_rcpdmiscs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_rcpdmisc) it.next());
        }        
    ...
    public Ar_rcpdmisc getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_rcpdmisc obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_rcpdmisc obj = (Ar_rcpdmisc) it.next();
    ...
    public String getDefaultSort() {
        return Ar_rcpdmiscs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_rcpdmiscs.SQLNAME;
    }
    ...
    public void searchData(double batchno, double docentry) throws CodeException {
        ...
        String query = " select * from ar_rcpdmisc where batchno = " + batchno + "  and docentry = " + docentry;
