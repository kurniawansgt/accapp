 ...
 *    -  Ar_billcycls objs = new Ar_billcycls(0); // All Data
 *    or 
 *    - Ar_billcycls obj = new Ar_billcycls(Ar_billcycl.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_billcycls obj = new Ar_billcycls(Ar_billcycl.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_billcycls brs = new Ar_billcycls(0);

            for (Iterator<Ar_billcycl> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_billcycl br = (Ar_billcycl) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_billcycl());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_billcycl br = (Ar_billcycl) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_billcycl());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ar_billcycl br = (Ar_billcycl) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_billcycl());
            }
 ...
 * @author windu
 */
public class Ar_billcycls  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_billcycls";
    public static final String SQLNAME = Ar_billcycl.TABLE;
    public static String DEFAULTSORT = Ar_billcycl.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_billcycl.PROPERTY_ID;
    ...
    List<Object> list;
    private Ar_billcycl parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ar_billcycl.PROPERTY_CYCLCODE, Ar_billcycl.PROPERTY_CYCLCODE, "200"},
        {Ar_billcycl.PROPERTY_DSCRITION, Ar_billcycl.PROPERTY_DSCRITION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_billcycls() throws CodeException {
        this (new Ar_billcycl(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_billcycls(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_billcycl(ID);
        ID_ = ID;
    }
    
    public Ar_billcycls(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_billcycl(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_billcycls(Ar_billcycl parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_billcycls(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_billcycls(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_billcycl obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_billcycls obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_billcycl) it.next());
        }        
    ...
    public Ar_billcycl getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_billcycl obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_billcycl obj = (Ar_billcycl) it.next();
    ...
    public String getDefaultSort() {
        return Ar_billcycls.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_billcycls.SQLNAME;
    }
