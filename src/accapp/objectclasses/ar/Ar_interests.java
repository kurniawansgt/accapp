 ...
 *    -  Ar_interests objs = new Ar_interests(0); // All Data
 *    or 
 *    - Ar_interests obj = new Ar_interests(Ar_interest.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_interests obj = new Ar_interests(Ar_interest.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_interests brs = new Ar_interests(0);

            for (Iterator<Ar_interest> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_interest br = (Ar_interest) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_interest());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_interest br = (Ar_interest) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_interest());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ar_interest br = (Ar_interest) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_interest());
            }
 ...
 * @author windu
 */
public class Ar_interests  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_interests";
    public static final String SQLNAME = Ar_interest.TABLE;
    public static String DEFAULTSORT = Ar_interest.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_interest.PROPERTY_ID;
    ...
    List<Object> list;
    private Ar_interest parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ar_interest.PROPERTY_INTRSCHRCODE, Ar_interest.PROPERTY_INTRSCHRCODE, "200"},
        {Ar_interest.PROPERTY_DSCRIPTION, Ar_interest.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_interests() throws CodeException {
        this (new Ar_interest(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_interests(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_interest(ID);
        ID_ = ID;
    }
    
    public Ar_interests(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_interest(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_interests(Ar_interest parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_interests(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_interests(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_interest obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_interests obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_interest) it.next());
        }        
    ...
    public Ar_interest getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_interest obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_interest obj = (Ar_interest) it.next();
    ...
    public String getDefaultSort() {
        return Ar_interests.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_interests.SQLNAME;
    }
