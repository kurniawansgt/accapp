 ...
 *    -  Ar_invbatchlists objs = new Ar_invbatchlists(0); // All Data
 *    or 
 *    - Ar_invbatchlists obj = new Ar_invbatchlists(Ar_invbatchlist.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_invbatchlists obj = new Ar_invbatchlists(Ar_invbatchlist.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_invbatchlists brs = new Ar_invbatchlists(0);

            for (Iterator<Ar_invbatchlist> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_invbatchlist br = (Ar_invbatchlist) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invbatchlist());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_invbatchlist br = (Ar_invbatchlist) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invbatchlist());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ar_invbatchlist br = (Ar_invbatchlist) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invbatchlist());
            }
 ...
 * @author windu
 */
public class Ar_invbatchlists  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_invbatchlists";
    public static final String SQLNAME = Ar_invbatchlist.TABLE;
    public static String DEFAULTSORT = Ar_invbatchlist.PROPERTY_BATCHNO;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_invbatchlist.PROPERTY_ID;
    ...
    List<Object> list;
    private Ar_invbatchlist parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ar_invbatchlist.PROPERTY_BATCHNO, "Batch", "50"},
        {Ar_invbatchlist.PROPERTY_BATCHDESC, "Description", "200"},
        {Ar_invbatchlist.PROPERTY_BATCHDATE, "Date", "80"},
        {Ar_invbatchlist.PROPERTY_INVENTRCNT, "Entry", "50"},
        {Ar_invbatchlist.PROPERTY_TOTINVAMT, "Amount", "150"},
        {Ar_invbatchlist.PROPERTY_BATCHSTS, "Status", "50"},
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_invbatchlists() throws CodeException {
        this (new Ar_invbatchlist(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_invbatchlists(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invbatchlist(ID);
        ID_ = ID;
    }
    
    public Ar_invbatchlists(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invbatchlist(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_invbatchlists(Ar_invbatchlist parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_invbatchlists(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_invbatchlists(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Ar_invbatchlist(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Ar_invbatchlist obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_invbatchlists obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_invbatchlist) it.next());
        }        
    ...
    public Ar_invbatchlist getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_invbatchlist obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_invbatchlist obj = (Ar_invbatchlist) it.next();
    ...
    public String getDefaultSort() {
        return Ar_invbatchlists.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_invbatchlists.SQLNAME;
    }
    ...
    public void searchData(String batchno) throws CodeException {
        ...
        if (batchno.equalsIgnoreCase("")){
            query = " select * from ar_invbatchlist order by batchno";
        } else {
            query = " select * from ar_invbatchlist where batchno>='"+batchno+"' order by batchno";
        }
        try {
