 ...
 *    -  Ar_distcodes objs = new Ar_distcodes(0); // All Data
 *    or 
 *    - Ar_distcodes obj = new Ar_distcodes(Ar_distcode.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_distcodes obj = new Ar_distcodes(Ar_distcode.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_distcodes brs = new Ar_distcodes(0);

            for (Iterator<Ar_distcode> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_distcode br = (Ar_distcode) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_distcode());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_distcode br = (Ar_distcode) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_distcode());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ar_distcode br = (Ar_distcode) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_distcode());
            }
 ...
 * @author windu
 */
public class Ar_distcodes  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_distcodes";
    public static final String SQLNAME = Ar_distcode.TABLE;
    public static String DEFAULTSORT = Ar_distcode.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_distcode.PROPERTY_ID;
    ...
    List<Object> list;
    private Ar_distcode parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ar_distcode.PRIMARYKEY, Ar_distcode.PRIMARYKEY, "200"}, 
        {Ar_distcode.PROPERTY_DSCRIPTION, Ar_distcode.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_distcodes() throws CodeException {
        this (new Ar_distcode(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_distcodes(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_distcode(ID);
        ID_ = ID;
    }
    
    public Ar_distcodes(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_distcode(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_distcodes(Ar_distcode parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_distcodes(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_distcodes(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_distcode obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_distcodes obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_distcode) it.next());
        }        
    ...
    public Ar_distcode getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_distcode obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_distcode obj = (Ar_distcode) it.next();
    ...
    public String getDefaultSort() {
        return Ar_distcodes.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_distcodes.SQLNAME;
    }
