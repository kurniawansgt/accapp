 ...
 *    -  Ar_invobss objs = new Ar_invobss(0); // All Data
 *    or 
 *    - Ar_invobss obj = new Ar_invobss(Ar_invobs.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_invobss obj = new Ar_invobss(Ar_invobs.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_invobss brs = new Ar_invobss(0);

            for (Iterator<Ar_invobs> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_invobs br = (Ar_invobs) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invobs());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_invobs br = (Ar_invobs) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invobs());
            }
 ...
 * @author windu
 */
public class Ar_invobss  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_invobss";
    public static final String SQLNAME = Ar_invobs.TABLE;
    public static String DEFAULTSORT = Ar_invobs.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_invobs.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_invobs.PRIMARYKEY, Ar_invobs.PRIMARYKEY, "200"}, 
        {Ar_invobs.PRIMARYKEY, Ar_invobs.PRIMARYKEY, "200"} 
    };
    ...
    public Ar_invobss() throws CodeException {
        this (new Ar_invobs(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_invobss(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invobs(ID);
        ID_ = ID;
    }
    
    public Ar_invobss(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invobs(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_invobss(Ar_invobs parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_invobss(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_invobss(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_invobs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_invobss obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_invobs) it.next());
        }        
    ...
    public Ar_invobs getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_invobs obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_invobs obj = (Ar_invobs) it.next();
    ...
    public String getDefaultSort() {
        return Ar_invobss.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_invobss.SQLNAME;
    }
