 ...
 *    -  Ar_invobps objs = new Ar_invobps(0); // All Data
 *    or 
 *    - Ar_invobps obj = new Ar_invobps(Ar_invobp.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_invobps obj = new Ar_invobps(Ar_invobp.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_invobps brs = new Ar_invobps(0);

            for (Iterator<Ar_invobp> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_invobp br = (Ar_invobp) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invobp());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_invobp br = (Ar_invobp) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invobp());
            }
 ...
 * @author windu
 */
public class Ar_invobps  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_invobps";
    public static final String SQLNAME = Ar_invobp.TABLE;
    public static String DEFAULTSORT = Ar_invobp.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_invobp.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_invobp.PRIMARYKEY, Ar_invobp.PRIMARYKEY, "200"}, 
        {Ar_invobp.PRIMARYKEY, Ar_invobp.PRIMARYKEY, "200"} 
    };
    ...
    public Ar_invobps() throws CodeException {
        this (new Ar_invobp(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_invobps(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invobp(ID);
        ID_ = ID;
    }
    
    public Ar_invobps(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invobp(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_invobps(Ar_invobp parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_invobps(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_invobps(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_invobp obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_invobps obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_invobp) it.next());
        }        
    ...
    public Ar_invobp getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_invobp obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_invobp obj = (Ar_invobp) it.next();
    ...
    public String getDefaultSort() {
        return Ar_invobps.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_invobps.SQLNAME;
    }
