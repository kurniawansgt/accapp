 ...
 *    -  Ar_adjhs objs = new Ar_adjhs(0); // All Data
 *    or 
 *    - Ar_adjhs obj = new Ar_adjhs(Ar_adjh.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_adjhs obj = new Ar_adjhs(Ar_adjh.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_adjhs brs = new Ar_adjhs(0);

            for (Iterator<Ar_adjh> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_adjh br = (Ar_adjh) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_adjh());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_adjh br = (Ar_adjh) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_adjh());
            }
 ...
 * @author windu
 */
public class Ar_adjhs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_adjhs";
    public static final String SQLNAME = Ar_adjh.TABLE;
    public static String DEFAULTSORT = Ar_adjh.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_adjh.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_adjh.PRIMARYKEY, Ar_adjh.PRIMARYKEY, "200"}, 
        {Ar_adjh.PRIMARYKEY, Ar_adjh.PRIMARYKEY, "200"} 
    };
    ...
    public Ar_adjhs() throws CodeException {
        this (new Ar_adjh(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_adjhs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_adjh(ID);
        ID_ = ID;
    }
    
    public Ar_adjhs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_adjh(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_adjhs(Ar_adjh parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_adjhs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_adjhs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_adjh obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_adjhs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ar_adjh getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_adjh obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ar_adjhs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_adjhs.SQLNAME;
    }
