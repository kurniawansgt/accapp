 ...
 *    -  Ar_rcphps objs = new Ar_rcphps(0); // All Data
 *    or 
 *    - Ar_rcphps obj = new Ar_rcphps(Ar_rcphp.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_rcphps obj = new Ar_rcphps(Ar_rcphp.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_rcphps brs = new Ar_rcphps(0);

            for (Iterator<Ar_rcphp> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_rcphp br = (Ar_rcphp) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcphp());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_rcphp br = (Ar_rcphp) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcphp());
            }
 ...
 * @author windu
 */
public class Ar_rcphps  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_rcphps";
    public static final String SQLNAME = Ar_rcphp.TABLE;
    public static String DEFAULTSORT = Ar_rcphp.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_rcphp.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_rcphp.PRIMARYKEY, Ar_rcphp.PRIMARYKEY, "200"}, 
        {Ar_rcphp.PRIMARYKEY, Ar_rcphp.PRIMARYKEY, "200"} 
    };
    ...
    public Ar_rcphps() throws CodeException {
        this (new Ar_rcphp(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_rcphps(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_rcphp(ID);
        ID_ = ID;
    }
    
    public Ar_rcphps(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_rcphp(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_rcphps(Ar_rcphp parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_rcphps(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_rcphps(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_rcphp obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_rcphps obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_rcphp) it.next());
        }        
    ...
    public Ar_rcphp getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_rcphp obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ar_rcphps.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_rcphps.SQLNAME;
    }
