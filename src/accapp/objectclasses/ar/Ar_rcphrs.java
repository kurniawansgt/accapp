 ...
 *    -  Ar_rcphrs objs = new Ar_rcphrs(0); // All Data
 *    or 
 *    - Ar_rcphrs obj = new Ar_rcphrs(Ar_rcphr.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_rcphrs obj = new Ar_rcphrs(Ar_rcphr.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_rcphrs brs = new Ar_rcphrs(0);

            for (Iterator<Ar_rcphr> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_rcphr br = (Ar_rcphr) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcphr());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_rcphr br = (Ar_rcphr) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcphr());
            }
 ...
 * @author windu
 */
public class Ar_rcphrs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_rcphrs";
    public static final String SQLNAME = Ar_rcphr.TABLE;
    public static String DEFAULTSORT = Ar_rcphr.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_rcphr.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_rcphr.PRIMARYKEY, Ar_rcphr.PRIMARYKEY, "200"}, 
        {Ar_rcphr.PRIMARYKEY, Ar_rcphr.PRIMARYKEY, "200"} 
    };
    ...
    public Ar_rcphrs() throws CodeException {
        this (new Ar_rcphr(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_rcphrs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_rcphr(ID);
        ID_ = ID;
    }
    
    public Ar_rcphrs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_rcphr(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_rcphrs(Ar_rcphr parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_rcphrs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_rcphrs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_rcphr obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_rcphrs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_rcphr) it.next());
        }        
    ...
    public Ar_rcphr getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_rcphr obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ar_rcphrs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_rcphrs.SQLNAME;
    }
