 ...
 *    -  Ar_custgrps objs = new Ar_custgrps(0); // All Data
 *    or 
 *    - Ar_custgrps obj = new Ar_custgrps(Ar_custgrp.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_custgrps obj = new Ar_custgrps(Ar_custgrp.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_custgrps brs = new Ar_custgrps(0);

            for (Iterator<Ar_custgrp> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_custgrp br = (Ar_custgrp) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_custgrp());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_custgrp br = (Ar_custgrp) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_custgrp());
            }
 ...
 * @author windu
 */
public class Ar_custgrps  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_custgrps";
    public static final String SQLNAME = Ar_custgrp.TABLE;
    public static String DEFAULTSORT = Ar_custgrp.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_custgrp.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_custgrp.PRIMARYKEY, Ar_custgrp.PRIMARYKEY, "200"}, 
        {Ar_custgrp.PROPERTY_DSCRIPTION, Ar_custgrp.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_custgrps() throws CodeException {
        this (new Ar_custgrp(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_custgrps(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_custgrp(ID);
        ID_ = ID;
    }
    
    public Ar_custgrps(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_custgrp(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_custgrps(Ar_custgrp parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_custgrps(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_custgrps(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_custgrp obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_custgrps obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_custgrp) it.next());
        }        
    ...
    public Ar_custgrp getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_custgrp obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_custgrp obj = (Ar_custgrp) it.next();
    ...
    public String getDefaultSort() {
        return Ar_custgrps.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_custgrps.SQLNAME;
    }
