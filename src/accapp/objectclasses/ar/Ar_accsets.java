 ...
 *    -  Ar_accsets objs = new Ar_accsets(0); // All Data
 *    or 
 *    - Ar_accsets obj = new Ar_accsets(Ar_accset.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_accsets obj = new Ar_accsets(Ar_accset.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_accsets brs = new Ar_accsets(0);

            for (Iterator<Ar_accset> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_accset br = (Ar_accset) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_accset());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_accset br = (Ar_accset) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_accset());
            }
 ...
 * @author windu
 */
public class Ar_accsets  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_accsets";
    public static final String SQLNAME = Ar_accset.TABLE;
    public static String DEFAULTSORT = Ar_accset.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_accset.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_accset.PROPERTY_ACCSETCODE, Ar_accset.PROPERTY_ACCSETCODE, "120"},
        {Ar_accset.PROPERTY_DSCRIPTION, Ar_accset.PROPERTY_DSCRIPTION, "250"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_accsets() throws CodeException {
        this (new Ar_accset(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_accsets(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_accset(ID);
        ID_ = ID;
    }
    
    public Ar_accsets(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_accset(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_accsets(Ar_accset parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_accsets(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_accsets(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_accset obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_accsets obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_accset) it.next());
        }        
    ...
    public Ar_accset getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_accset obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_accset obj = (Ar_accset) it.next();
    ...
    public String getDefaultSort() {
        return Ar_accsets.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_accsets.SQLNAME;
    }
