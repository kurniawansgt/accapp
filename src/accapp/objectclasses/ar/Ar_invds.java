 ...
 *    -  Ar_invds objs = new Ar_invds(0); // All Data
 *    or 
 *    - Ar_invds obj = new Ar_invds(Ar_invd.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_invds obj = new Ar_invds(Ar_invd.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_invds brs = new Ar_invds(0);

            for (Iterator<Ar_invd> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_invd br = (Ar_invd) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invd());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_invd br = (Ar_invd) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invd());
            }
 ...
 * @author windu
 */
public class Ar_invds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_invds";
    public static final String SQLNAME = Ar_invd.TABLE;
    public static String DEFAULTSORT = Ar_invd.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_invd.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_invd.PRIMARYKEY, Ar_invd.PRIMARYKEY, "200"}, 
        {Ar_invd.PRIMARYKEY, Ar_invd.PRIMARYKEY, "200"} 
    };
    ...
    public Ar_invds() throws CodeException {
        this (new Ar_invd(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_invds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invd(ID);
        ID_ = ID;
    }
    
    public Ar_invds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invd(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_invds(Ar_invd parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_invds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_invds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_invd obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_invds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ar_invd getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_invd obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ar_invds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_invds.SQLNAME;
    }
    ...
    public void searchData(double btchentry, double journalid) throws CodeException {
        ...
        String query = " select * from ar_invd where batchno = " + btchentry + "  and docentry = " + journalid;
