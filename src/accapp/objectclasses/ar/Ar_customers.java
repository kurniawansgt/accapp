 ...
 *    -  Ar_customers objs = new Ar_customers(0); // All Data
 *    or 
 *    - Ar_customers obj = new Ar_customers(Ar_customer.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_customers obj = new Ar_customers(Ar_customer.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_customers brs = new Ar_customers(0);

            for (Iterator<Ar_customer> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_customer br = (Ar_customer) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_customer());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_customer br = (Ar_customer) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_customer());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ar_customer br = (Ar_customer) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_customer());
            }
 ...
 * @author windu
 */
public class Ar_customers  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_customers";
    public static final String SQLNAME = Ar_customer.TABLE;
    public static String DEFAULTSORT = Ar_customer.PROPERTY_CUSTMRCODE;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_customer.PROPERTY_ID;
    ...
    List<Object> list;
    private Ar_customer parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ar_customer.PRIMARYKEY, Ar_customer.PRIMARYKEY, "200"}, 
        {Ar_customer.PROPERTY_CUSTMRNAME, Ar_customer.PROPERTY_CUSTMRNAME, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_customers() throws CodeException {
        this (new Ar_customer(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_customers(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_customer(ID);
        ID_ = ID;
    }
    
    public Ar_customers(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_customer(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_customers(Ar_customer parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_customers(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_customers(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_customer obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_customers obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_customer) it.next());
        }        
    ...
    public Ar_customer getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_customer obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_customer obj = (Ar_customer) it.next();
    ...
    public String getDefaultSort() {
        return Ar_customers.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_customers.SQLNAME;
    }
