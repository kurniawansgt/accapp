 ...
 *    -  Ar_invobls objs = new Ar_invobls(0); // All Data
 *    or 
 *    - Ar_invobls obj = new Ar_invobls(Ar_invobl.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_invobls obj = new Ar_invobls(Ar_invobl.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_invobls brs = new Ar_invobls(0);

            for (Iterator<Ar_invobl> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_invobl br = (Ar_invobl) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invobl());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_invobl br = (Ar_invobl) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invobl());
            }
 ...
 * @author windu
 */
public class Ar_invobls  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_invobls";
    public static final String SQLNAME = Ar_invobl.TABLE;
    public static String DEFAULTSORT = Ar_invobl.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_invobl.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_invobl.PROPERTY_DOCNUM, Ar_invobl.PROPERTY_DOCNUM, "100"},
        {Ar_invobl.PROPERTY_DOCTYPE, Ar_invobl.PROPERTY_DOCTYPE, "100"},
        {Ar_invobl.PROPERTY_DOCTOTALAMTR, Ar_invobl.PROPERTY_DOCTOTALAMTR, "100"},
        {Ar_invobl.PROPERTY_CURNCCODE, Ar_invobl.PROPERTY_CURNCCODE, "100"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_invobls() throws CodeException {
        this (new Ar_invobl(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_invobls(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invobl(ID);
        ID_ = ID;
    }
    
    public Ar_invobls(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invobl(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_invobls(Ar_invobl parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_invobls(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_invobls(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_invobl obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_invobls obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_invobl) it.next());
        }        
    ...
    public Ar_invobl getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_invobl obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_invobl obj = (Ar_invobl) it.next();
    ...
    public String getDefaultSort() {
        return Ar_invobls.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_invobls.SQLNAME;
    }


    public void searchData(String custmrcode,String mode,String startpono,String type,String sortby) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();

        String query="SELECT * FROM ar_invobl "
                + "WHERE custmrcode='"+custmrcode+"' AND swpaid=0 "
                + "AND doctype=CASE WHEN "+type+"='' THEN doctype ELSE "+type+" END "
    ...
    public void searchDataAdjust(String vendcode, String doctype) throws CodeException {
        ...
        String query = " Select * from ar_invobl where doctype=" + doctype + " and custmrcode='" + vendcode + "' And (swpaid=0  or  doctotalamtr=0 ) ";
        try {
            rs = db.executeQuery(query);
    ...
    public void searchDataAdjust(String vendcode) throws CodeException {
        ...
        String query = " Select * from ar_invobl where custmrcode='" + vendcode + "' ";
        try {
            rs = db.executeQuery(query);
