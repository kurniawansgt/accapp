 ...
 *    -  Ar_invhs objs = new Ar_invhs(0); // All Data
 *    or 
 *    - Ar_invhs obj = new Ar_invhs(Ar_invh.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_invhs obj = new Ar_invhs(Ar_invh.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_invhs brs = new Ar_invhs(0);

            for (Iterator<Ar_invh> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_invh br = (Ar_invh) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invh());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_invh br = (Ar_invh) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_invh());
            }
 ...
 * @author windu
 */
public class Ar_invhs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_invhs";
    public static final String SQLNAME = Ar_invh.TABLE;
    public static String DEFAULTSORT = Ar_invh.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_invh.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_invh.PRIMARYKEY, Ar_invh.PRIMARYKEY, "200"}, 
        {Ar_invh.PROPERTY_DOCNUM, Ar_invh.PROPERTY_DOCNUM, "300"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_invhs() throws CodeException {
        this (new Ar_invh(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_invhs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invh(ID);
        ID_ = ID;
    }
    
    public Ar_invhs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_invh(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_invhs(Ar_invh parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_invhs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_invhs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_invh obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_invhs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Ar_invh getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_invh obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Ar_invhs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_invhs.SQLNAME;
    }


    public void searchData(String startno,String type,String sortby) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();

        String query = " select * from ar_invh where docnum = " + startno + "  and doctype = " + type +" order by "+sortby;
