 ...
 *    -  Ar_custnats objs = new Ar_custnats(0); // All Data
 *    or 
 *    - Ar_custnats obj = new Ar_custnats(Ar_custnat.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_custnats obj = new Ar_custnats(Ar_custnat.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_custnats brs = new Ar_custnats(0);

            for (Iterator<Ar_custnat> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_custnat br = (Ar_custnat) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_custnat());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_custnat br = (Ar_custnat) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_custnat());
            }
 ...
 * @author windu
 */
public class Ar_custnats  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_custnats";
    public static final String SQLNAME = Ar_custnat.TABLE;
    public static String DEFAULTSORT = Ar_custnat.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_custnat.PROPERTY_ID;
    ...
    private String[][] column = {
        {Ar_custnat.PRIMARYKEY, Ar_custnat.PRIMARYKEY, "200"}, 
        {Ar_custnat.PROPERTY_DSCRIPTION, Ar_custnat.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_custnats() throws CodeException {
        this (new Ar_custnat(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_custnats(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_custnat(ID);
        ID_ = ID;
    }
    
    public Ar_custnats(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_custnat(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_custnats(Ar_custnat parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_custnats(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_custnats(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Ar_custnat obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_custnats obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_custnat) it.next());
        }        
    ...
    public Ar_custnat getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_custnat obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_custnat obj = (Ar_custnat) it.next();
    ...
    public String getDefaultSort() {
        return Ar_custnats.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_custnats.SQLNAME;
    }
