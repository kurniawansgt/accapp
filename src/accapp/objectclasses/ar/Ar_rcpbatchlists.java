 ...
 *    -  Ar_rcpbatchlists objs = new Ar_rcpbatchlists(0); // All Data
 *    or 
 *    - Ar_rcpbatchlists obj = new Ar_rcpbatchlists(Ar_rcpbatchlist.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_rcpbatchlists obj = new Ar_rcpbatchlists(Ar_rcpbatchlist.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_rcpbatchlists brs = new Ar_rcpbatchlists(0);

            for (Iterator<Ar_rcpbatchlist> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_rcpbatchlist br = (Ar_rcpbatchlist) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcpbatchlist());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_rcpbatchlist br = (Ar_rcpbatchlist) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcpbatchlist());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ar_rcpbatchlist br = (Ar_rcpbatchlist) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_rcpbatchlist());
            }
 ...
 * @author windu
 */
public class Ar_rcpbatchlists  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_rcpbatchlists";
    public static final String SQLNAME = Ar_rcpbatchlist.TABLE;
    public static String DEFAULTSORT = Ar_rcpbatchlist.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_rcpbatchlist.PROPERTY_ID;
    ...
    List<Object> list;
    private Ar_rcpbatchlist parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ar_rcpbatchlist.PRIMARYKEY, Ar_rcpbatchlist.PRIMARYKEY, "80"},
//        {Ar_rcpbatchlist.PRIMARYKEY, Ar_rcpbatchlist.PRIMARYKEY, "200"},
        {Ar_rcpbatchlist.PROPERTY_BATCHDESC, "BATCH DESCRIPTION", "200"},
        {Ar_rcpbatchlist.PROPERTY_BATCHDATE, "BATCH DATE", "80"},
        {Ar_rcpbatchlist.PROPERTY_GLPOSTNGSEQ, "GL POST SEQ NO", "125"},
        {Ar_rcpbatchlist.PROPERTY_TOTRCPAMT , "BATCH AMOUNT", "150"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Ar_rcpbatchlists() throws CodeException {
        this (new Ar_rcpbatchlist(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_rcpbatchlists(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_rcpbatchlist(ID);
        ID_ = ID;
    }
    
    public Ar_rcpbatchlists(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_rcpbatchlist(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_rcpbatchlists(Ar_rcpbatchlist parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_rcpbatchlists(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_rcpbatchlists(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Ar_rcpbatchlist(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Ar_rcpbatchlist obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_rcpbatchlists obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_rcpbatchlist) it.next());
        }        
    ...
    public Ar_rcpbatchlist getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_rcpbatchlist obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_rcpbatchlist obj = (Ar_rcpbatchlist) it.next();
    ...
    public String getDefaultSort() {
        return Ar_rcpbatchlists.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_rcpbatchlists.SQLNAME;
    }
    ...
    public void searchData(String batchno) throws CodeException {
        ...
        if (batchno.equalsIgnoreCase("")){
            query = " select * from ar_rcpbatchlist order by batchno";
        } else {
            query = " select * from ar_rcpbatchlist where batchno>='"+batchno+"' order by batchno";
        }
        try {
