 ...
 *    -  Ar_customerstss objs = new Ar_customerstss(0); // All Data
 *    or 
 *    - Ar_customerstss obj = new Ar_customerstss(Ar_customersts.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ar_customerstss obj = new Ar_customerstss(Ar_customersts.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ar_customerstss brs = new Ar_customerstss(0);

            for (Iterator<Ar_customersts> it = brs.iterator(); it.hasNext(); ) {
                
                Ar_customersts br = (Ar_customersts) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_customersts());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ar_customersts br = (Ar_customersts) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_customersts());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ar_customersts br = (Ar_customersts) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaAr_customersts());
            }
 ...
 * @author windu
 */
public class Ar_customerstss  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ar_customerstss";
    public static final String SQLNAME = Ar_customersts.TABLE;
    public static String DEFAULTSORT = Ar_customersts.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ar_customersts.PROPERTY_ID;
    ...
    List<Object> list;
    private Ar_customersts parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ar_customersts.PRIMARYKEY, Ar_customersts.PRIMARYKEY, "200"}, 
        {Ar_customersts.PRIMARYKEY, Ar_customersts.PRIMARYKEY, "200"} 
    };
    ...
    public Ar_customerstss() throws CodeException {
        this (new Ar_customersts(NULLID));
        _isLoaded_ = true;
    }
    
    public Ar_customerstss(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_customersts(ID);
        ID_ = ID;
    }
    
    public Ar_customerstss(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ar_customersts(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ar_customerstss(Ar_customersts parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ar_customerstss(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ar_customerstss(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Ar_customersts(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Ar_customersts obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ar_customerstss obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ar_customersts) it.next());
        }        
    ...
    public Ar_customersts getParent() {
        return parentObj_;
    }
    
    public void setParent(Ar_customersts obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ar_customersts obj = (Ar_customersts) it.next();
    ...
    public String getDefaultSort() {
        return Ar_customerstss.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ar_customerstss.SQLNAME;
    }
