 ...
 *    -  Cb_distds objs = new Cb_distds(0); // All Data
 *    or 
 *    - Cb_distds obj = new Cb_distds(Cb_distd.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_distds obj = new Cb_distds(Cb_distd.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_distds brs = new Cb_distds(0);

            for (Iterator<Cb_distd> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_distd br = (Cb_distd) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_distd());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_distd br = (Cb_distd) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_distd());
            }
 ...
 * @author windu
 */
public class Cb_distds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_distds";
    public static final String SQLNAME = Cb_distd.TABLE;
    public static String DEFAULTSORT = Cb_distd.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_distd.PROPERTY_ID;
    ...
    private String[][] column = {
        {Cb_distd.PRIMARYKEY, Cb_distd.PRIMARYKEY, "200"}, 
        {Cb_distd.PRIMARYKEY, Cb_distd.PRIMARYKEY, "200"} 
    };
    ...
    public Cb_distds() throws CodeException {
        this (new Cb_distd(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_distds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_distd(ID);
        ID_ = ID;
    }
    
    public Cb_distds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_distd(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_distds(Cb_distd parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_distds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_distds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_distd obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_distds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_distd) it.next());
        }        
    ...
    public Cb_distd getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_distd obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Cb_distds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_distds.SQLNAME;
    }
