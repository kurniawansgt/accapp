 ...
 *    -  Cb_voucherds objs = new Cb_voucherds(0); // All Data
 *    or
 *    - Cb_voucherds obj = new Cb_voucherds(Cb_voucherd.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_voucherds obj = new Cb_voucherds(Cb_voucherd.PROPERTY_ID, "1");
 ...
 * 2. Read Data
        try {
            ...
            Cb_voucherds brs = new Cb_voucherds(0);

            for (Iterator<Cb_voucherd> it = brs.iterator(); it.hasNext(); ) {

                Cb_voucherd br = (Cb_voucherd) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_voucherd());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_voucherd br = (Cb_voucherd) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_voucherd());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Cb_voucherd br = (Cb_voucherd) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_voucherd());
            }
 ...
 * @author windu
 */
public class Cb_voucherds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_voucherds";
    public static final String SQLNAME = Cb_voucherd.TABLE;
    public static String DEFAULTSORT = Cb_voucherd.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";

    public static final String FOREIGNKEY = Cb_voucherd.PROPERTY_ID;
    ...
    List<Object> list;
    private Cb_voucherd parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Cb_voucherd.PRIMARYKEY, Cb_voucherd.PRIMARYKEY, "200"},
        {Cb_voucherd.PRIMARYKEY, Cb_voucherd.PRIMARYKEY, "200"}
    };
    ...
    public Cb_voucherds() throws CodeException {
        this (new Cb_voucherd(NULLID));
        _isLoaded_ = true;
    }

    public Cb_voucherds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_voucherd(ID);
        ID_ = ID;
    }

    public Cb_voucherds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_voucherd(PrimaryKey);
        ID_ = parentObj_.getID();
    }

    public Cb_voucherds(Cb_voucherd parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_voucherds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }

    public Cb_voucherds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_voucherd obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_voucherds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_voucherd) it.next());
        }
    ...
    public Cb_voucherd getParent() {
        return parentObj_;
    }

    public void setParent(Cb_voucherd obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Cb_voucherd obj = (Cb_voucherd) it.next();
    ...
    public String getDefaultSort() {
        return Cb_voucherds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_voucherds.SQLNAME;
    }
    ...
    public void searchData(double batchno, long entryno) throws CodeException {
        ...
        String query = " select * from cb_voucherd where batchno = " + batchno + "  and entryno = " + entryno;
