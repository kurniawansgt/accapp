 ...
 *    -  Payment_vouchers objs = new Payment_vouchers(0); // All Data
 *    or 
 *    - Payment_vouchers obj = new Payment_vouchers(Payment_voucher.PROPERTY_<field>, <value>);
 *      Sample :
 *      Payment_vouchers obj = new Payment_vouchers(Payment_voucher.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Payment_vouchers brs = new Payment_vouchers(0);

            for (Iterator<Payment_voucher> it = brs.iterator(); it.hasNext(); ) {
                
                Payment_voucher br = (Payment_voucher) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaPayment_voucher());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Payment_voucher br = (Payment_voucher) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaPayment_voucher());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Payment_voucher br = (Payment_voucher) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaPayment_voucher());
            }
 ...
 * @author windu
 */
public class Payment_vouchers  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Payment_vouchers";
    public static final String SQLNAME = Payment_voucher.TABLE;
    public static String DEFAULTSORT = Payment_voucher.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Payment_voucher.PROPERTY_ID;
    ...
    List<Object> list;
    private Payment_voucher parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Payment_voucher.PRIMARYKEY, Payment_voucher.PRIMARYKEY, "100"}, 
        {Payment_voucher.PROPERTY_CASHFLOW_NO, Payment_voucher.PROPERTY_CASHFLOW_NO, "100"},
        {Payment_voucher.PROPERTY_PV_AMOUNT, Payment_voucher.PROPERTY_PV_AMOUNT, "130"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Payment_vouchers() throws CodeException {
        this (new Payment_voucher(NULLID));
        _isLoaded_ = true;
    }
    
    public Payment_vouchers(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Payment_voucher(ID);
        ID_ = ID;
    }
    
    public Payment_vouchers(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Payment_voucher(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Payment_vouchers(Payment_voucher parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Payment_vouchers(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Payment_vouchers(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Payment_voucher(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Payment_voucher obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Payment_vouchers obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Payment_voucher) it.next());
        }        
    ...
    public Payment_voucher getParent() {
        return parentObj_;
    }
    
    public void setParent(Payment_voucher obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Payment_voucher obj = (Payment_voucher) it.next();
    ...
    public String getDefaultSort() {
        return Payment_vouchers.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Payment_vouchers.SQLNAME;
    }
