 ...
 *    -  Cb_cash_advances objs = new Cb_cash_advances(0); // All Data
 *    or 
 *    - Cb_cash_advances obj = new Cb_cash_advances(Cb_cash_advance.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_cash_advances obj = new Cb_cash_advances(Cb_cash_advance.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_cash_advances brs = new Cb_cash_advances(0);

            for (Iterator<Cb_cash_advance> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_cash_advance br = (Cb_cash_advance) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_cash_advance());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_cash_advance br = (Cb_cash_advance) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_cash_advance());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Cb_cash_advance br = (Cb_cash_advance) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_cash_advance());
            }
 ...
 * @author windu
 */
public class Cb_cash_advances  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_cash_advances";
    public static final String SQLNAME = Cb_cash_advance.TABLE;
    public static String DEFAULTSORT = Cb_cash_advance.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_cash_advance.PROPERTY_ID;
    ...
    List<Object> list;
    private Cb_cash_advance parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Cb_cash_advance.PROPERTY_ADVANCE_NO, Cb_cash_advance.PROPERTY_ADVANCE_NO, "100"},
        {Cb_cash_advance.PROPERTY_ADVANCE_REF_NO, Cb_cash_advance.PROPERTY_ADVANCE_REF_NO, "100"},
        {Cb_cash_advance.PROPERTY_ADVANCE_DATE, Cb_cash_advance.PROPERTY_ADVANCE_DATE, "100"},
        {Cb_cash_advance.PROPERTY_ADVANCE_PIC, Cb_cash_advance.PROPERTY_ADVANCE_PIC, "100"},
        {Cb_cash_advance.PROPERTY_DSCRIPTION, Cb_cash_advance.PROPERTY_DSCRIPTION, "100"},
        {Cb_cash_advance.PROPERTY_ADVANCE_AMOUNT, Cb_cash_advance.PROPERTY_ADVANCE_AMOUNT, "100"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Cb_cash_advances() throws CodeException {
        this (new Cb_cash_advance(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_cash_advances(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_cash_advance(ID);
        ID_ = ID;
    }
    
    public Cb_cash_advances(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_cash_advance(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_cash_advances(Cb_cash_advance parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_cash_advances(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_cash_advances(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Cb_cash_advance(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Cb_cash_advance obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_cash_advances obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_cash_advance) it.next());
        }        
    ...
    public Cb_cash_advance getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_cash_advance obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Cb_cash_advance obj = (Cb_cash_advance) it.next();
    ...
    public String getDefaultSort() {
        return Cb_cash_advances.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_cash_advances.SQLNAME;
    }
