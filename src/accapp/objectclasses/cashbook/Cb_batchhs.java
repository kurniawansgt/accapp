 ...
 *    -  Cb_batchhs objs = new Cb_batchhs(0); // All Data
 *    or 
 *    - Cb_batchhs obj = new Cb_batchhs(Cb_batchh.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_batchhs obj = new Cb_batchhs(Cb_batchh.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_batchhs brs = new Cb_batchhs(0);

            for (Iterator<Cb_batchh> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_batchh br = (Cb_batchh) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_batchh());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_batchh br = (Cb_batchh) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_batchh());
            }
 ...
 * @author windu
 */
public class Cb_batchhs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_batchhs";
    public static final String SQLNAME = Cb_batchh.TABLE;
    public static String DEFAULTSORT = Cb_batchh.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_batchh.PROPERTY_ID;
    ...
    public String[][] column = {
        {Cb_batchh.PROPERTY_ENTRYNO, Cb_batchh.PROPERTY_ENTRYNO, "70"},
    ...
    public Cb_batchhs() throws CodeException {
        this (new Cb_batchh(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_batchhs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_batchh(ID);
        ID_ = ID;
    }
    
    public Cb_batchhs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_batchh(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_batchhs(Cb_batchh parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_batchhs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_batchhs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_batchh obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_batchhs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_batchh) it.next());
        }        
    ...
    public Cb_batchh getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_batchh obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Cb_batchh obj = (Cb_batchh) it.next();
    ...
    public String getDefaultSort() {
        return Cb_batchhs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_batchhs.SQLNAME;
    }
