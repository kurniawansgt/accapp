 ...
 *    -  Cb_pjds objs = new Cb_pjds(0); // All Data
 *    or 
 *    - Cb_pjds obj = new Cb_pjds(Cb_pjd.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_pjds obj = new Cb_pjds(Cb_pjd.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_pjds brs = new Cb_pjds(0);

            for (Iterator<Cb_pjd> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_pjd br = (Cb_pjd) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_pjd());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_pjd br = (Cb_pjd) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_pjd());
            }
 ...
 * @author windu
 */
public class Cb_pjds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_pjds";
    public static final String SQLNAME = Cb_pjd.TABLE;
    public static String DEFAULTSORT = Cb_pjd.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_pjd.PROPERTY_ID;
    ...
    private String[][] column = {
        {Cb_pjd.PRIMARYKEY, Cb_pjd.PRIMARYKEY, "200"}, 
        {Cb_pjd.PRIMARYKEY, Cb_pjd.PRIMARYKEY, "200"} 
    };
    ...
    public Cb_pjds() throws CodeException {
        this (new Cb_pjd(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_pjds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_pjd(ID);
        ID_ = ID;
    }
    
    public Cb_pjds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_pjd(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_pjds(Cb_pjd parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_pjds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_pjds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_pjd obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_pjds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Cb_pjd getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_pjd obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Cb_pjds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_pjds.SQLNAME;
    }


    public void searchData(Double batchno,Double entryno) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();

        String query = " select * from cb_pjd where batchno='"+batchno+"' and entryno='"+entryno+"'";
    ...
    public void searchData(Double batchno) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();

        String query = " select * from cb_pjd where batchno='"+batchno+"' order by entryno";
