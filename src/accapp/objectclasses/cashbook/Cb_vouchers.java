 ...
 *    -  Cb_vouchers objs = new Cb_vouchers(0); // All Data
 *    or 
 *    - Cb_vouchers obj = new Cb_vouchers(Cb_voucher.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_vouchers obj = new Cb_vouchers(Cb_voucher.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_vouchers brs = new Cb_vouchers(0);

            for (Iterator<Cb_voucher> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_voucher br = (Cb_voucher) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_voucher());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_voucher br = (Cb_voucher) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_voucher());
            }
 ...
 * @author windu
 */
public class Cb_vouchers  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_vouchers";
    public static final String SQLNAME = Cb_voucher.TABLE;
    public static String DEFAULTSORT = Cb_voucher.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_voucher.PROPERTY_ID;
    ...
    private String[][] column = {
        {Cb_voucher.PROPERTY_ENTRYNO, Cb_voucher.PROPERTY_ENTRYNO, "200"},
        {Cb_voucher.PROPERTY_REFERENCE, Cb_voucher.PROPERTY_REFERENCE, "200"} ,
        {Cb_voucher.PROPERTY_MISCCODE, Cb_voucher.PROPERTY_MISCCODE, "200"} ,
        {Cb_voucher.PROPERTY_DSCRIPTION, Cb_voucher.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Cb_vouchers() throws CodeException {
        this (new Cb_voucher(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_vouchers(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_voucher(ID);
        ID_ = ID;
    }
    
    public Cb_vouchers(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_voucher(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_vouchers(Cb_voucher parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_vouchers(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_vouchers(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_voucher obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_vouchers obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_voucher) it.next());
        }        
    ...
    public Cb_voucher getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_voucher obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Cb_voucher obj = (Cb_voucher) it.next();
    ...
    public String getDefaultSort() {
        return Cb_vouchers.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_vouchers.SQLNAME;
    }
