 ...
 *    -  Cb_disths objs = new Cb_disths(0); // All Data
 *    or 
 *    - Cb_disths obj = new Cb_disths(Cb_disth.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_disths obj = new Cb_disths(Cb_disth.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_disths brs = new Cb_disths(0);

            for (Iterator<Cb_disth> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_disth br = (Cb_disth) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_disth());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_disth br = (Cb_disth) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_disth());
            }
 ...
 * @author windu
 */
public class Cb_disths  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_disths";
    public static final String SQLNAME = Cb_disth.TABLE;
    public static String DEFAULTSORT = Cb_disth.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_disth.PROPERTY_ID;
    ...
    private String[][] column = {
        {Cb_disth.PRIMARYKEY, Cb_disth.PRIMARYKEY, "200"}, 
        {Cb_disth.PROPERTY_DSCRIPTION, Cb_disth.PROPERTY_DSCRIPTION, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Cb_disths() throws CodeException {
        this (new Cb_disth(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_disths(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_disth(ID);
        ID_ = ID;
    }
    
    public Cb_disths(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_disth(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_disths(Cb_disth parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_disths(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_disths(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_disth obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_disths obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_disth) it.next());
        }        
    ...
    public Cb_disth getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_disth obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Cb_disths.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_disths.SQLNAME;
    }
