 ...
 *    -  Cb_sources objs = new Cb_sources(0); // All Data
 *    or 
 *    - Cb_sources obj = new Cb_sources(Cb_source.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_sources obj = new Cb_sources(Cb_source.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_sources brs = new Cb_sources(0);

            for (Iterator<Cb_source> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_source br = (Cb_source) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_source());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_source br = (Cb_source) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_source());
            }
 ...
 * @author windu
 */
public class Cb_sources  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_sources";
    public static final String SQLNAME = Cb_source.TABLE;
    public static String DEFAULTSORT = Cb_source.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_source.PROPERTY_ID;
    ...
    private String[][] column = {
        {Cb_source.PRIMARYKEY, Cb_source.PRIMARYKEY, "150"},
        {Cb_source.PROPERTY_DSCRIPTION, Cb_source.PROPERTY_DSCRIPTION, "250"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Cb_sources() throws CodeException {
        this (new Cb_source(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_sources(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_source(ID);
        ID_ = ID;
    }
    
    public Cb_sources(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_source(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_sources(Cb_source parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_sources(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_sources(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_source obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_sources obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_source) it.next());
        }        
    ...
    public Cb_source getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_source obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Cb_source obj = (Cb_source) it.next();
    ...
    public String getDefaultSort() {
        return Cb_sources.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_sources.SQLNAME;
    }
