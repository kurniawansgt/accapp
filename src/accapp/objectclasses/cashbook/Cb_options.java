 ...
 *    -  Cb_options objs = new Cb_options(0); // All Data
 *    or 
 *    - Cb_options obj = new Cb_options(Cb_option.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_options obj = new Cb_options(Cb_option.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_options brs = new Cb_options(0);

            for (Iterator<Cb_option> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_option br = (Cb_option) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_option());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_option br = (Cb_option) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_option());
            }
 ...
 * @author windu
 */
public class Cb_options  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_options";
    public static final String SQLNAME = Cb_option.TABLE;
    public static String DEFAULTSORT = Cb_option.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_option.PROPERTY_ID;
    ...
    private String[][] column = {
        {Cb_option.PRIMARYKEY, Cb_option.PRIMARYKEY, "200"}, 
        {Cb_option.PRIMARYKEY, Cb_option.PRIMARYKEY, "200"} 
    };
    ...
    public Cb_options() throws CodeException {
        this (new Cb_option(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_options(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_option(ID);
        ID_ = ID;
    }
    
    public Cb_options(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_option(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_options(Cb_option parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_options(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_options(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_option obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_options obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_option) it.next());
        }        
    ...
    public Cb_option getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_option obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Cb_option obj = (Cb_option) it.next();
    ...
    public String getDefaultSort() {
        return Cb_options.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_options.SQLNAME;
    }
