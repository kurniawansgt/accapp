 ...
 *    -  Cb_batchcontrols objs = new Cb_batchcontrols(0); // All Data
 *    or 
 *    - Cb_batchcontrols obj = new Cb_batchcontrols(Cb_batchcontrol.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_batchcontrols obj = new Cb_batchcontrols(Cb_batchcontrol.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_batchcontrols brs = new Cb_batchcontrols(0);

            for (Iterator<Cb_batchcontrol> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_batchcontrol br = (Cb_batchcontrol) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_batchcontrol());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_batchcontrol br = (Cb_batchcontrol) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_batchcontrol());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Cb_batchcontrol br = (Cb_batchcontrol) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_batchcontrol());
            }
 ...
 * @author windu
 */
public class Cb_batchcontrols  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_batchcontrols";
    public static final String SQLNAME = Cb_batchcontrol.TABLE;
    public static String DEFAULTSORT = Cb_batchcontrol.PROPERTY_BATCHNO;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_batchcontrol.PROPERTY_ID;
    ...
    List<Object> list;
    private Cb_batchcontrol parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Cb_batchcontrol.PRIMARYKEY, Cb_batchcontrol.PRIMARYKEY, "100"},
        {Cb_batchcontrol.PROPERTY_BANKCODE, Cb_batchcontrol.PROPERTY_BANKCODE, "100"},
        {Cb_batchcontrol.PROPERTY_BTCHDESC, Cb_batchcontrol.PROPERTY_BTCHDESC, "200"},
        {Cb_batchcontrol.PROPERTY_ENTRYTYPE, Cb_batchcontrol.PROPERTY_ENTRYTYPE, "100"},
        {Cb_batchcontrol.PROPERTY_BATCHSTS, Cb_batchcontrol.PROPERTY_BATCHSTS, "100"},
        {Cb_batchcontrol.PROPERTY_BATCHDATE, Cb_batchcontrol.PROPERTY_BATCHDATE, "100"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Cb_batchcontrols() throws CodeException {
        this (new Cb_batchcontrol(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_batchcontrols(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_batchcontrol(ID);
        ID_ = ID;
    }
    
    public Cb_batchcontrols(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_batchcontrol(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_batchcontrols(Cb_batchcontrol parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_batchcontrols(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_batchcontrols(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Cb_batchcontrol(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Cb_batchcontrol obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_batchcontrols obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Cb_batchcontrol) it.next());
        }        
    ...
    public Cb_batchcontrol getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_batchcontrol obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Cb_batchcontrol obj = (Cb_batchcontrol) it.next();
    ...
    public String getDefaultSort() {
        return Cb_batchcontrols.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_batchcontrols.SQLNAME;
    }

    public void searchData(String frbatchno,String tobatchno,String frbankcode,String tobankcode) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();

        String query = "select * from cb_batchcontrol a where  batchsts='O' and "
                + "\n batchno between case when '"+frbatchno+"'='' then batchno else '"+frbatchno+"' end "
                + "\n and case when '"+tobatchno+"'='' then batchno else '"+tobatchno+"' end "
                + "\n and bankcode between case when '"+frbankcode+"'='' then bankcode else '"+frbankcode+"' end "
                + "\n and case when '"+tobankcode+"'='' then bankcode else '"+tobankcode+"' end "
                + "\n and exists (select id from cb_batchh b where batchno=a.batchno and exists "
                + "\n (select id from cb_batchd where batchno=b.batchno and entryno=b.entryno))";
    ...
    public void searchDataforGL(String frbankcode,String tobankcode,String frdate,String todate) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();

        String query = "select * from cb_batchcontrol "
                + "where batchsts='P' and postngseq<>0 "
                + "group by postngseq";
