 ...
 *    -  Cb_pjsds objs = new Cb_pjsds(0); // All Data
 *    or 
 *    - Cb_pjsds obj = new Cb_pjsds(Cb_pjsd.PROPERTY_<field>, <value>);
 *      Sample :
 *      Cb_pjsds obj = new Cb_pjsds(Cb_pjsd.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Cb_pjsds brs = new Cb_pjsds(0);

            for (Iterator<Cb_pjsd> it = brs.iterator(); it.hasNext(); ) {
                
                Cb_pjsd br = (Cb_pjsd) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_pjsd());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Cb_pjsd br = (Cb_pjsd) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCb_pjsd());
            }
 ...
 * @author windu
 */
public class Cb_pjsds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Cb_pjsds";
    public static final String SQLNAME = Cb_pjsd.TABLE;
    public static String DEFAULTSORT = Cb_pjsd.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Cb_pjsd.PROPERTY_ID;
    ...
    private String[][] column = {
        {Cb_pjsd.PRIMARYKEY, Cb_pjsd.PRIMARYKEY, "200"}, 
        {Cb_pjsd.PRIMARYKEY, Cb_pjsd.PRIMARYKEY, "200"} 
    };
    ...
    public Cb_pjsds() throws CodeException {
        this (new Cb_pjsd(NULLID));
        _isLoaded_ = true;
    }
    
    public Cb_pjsds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_pjsd(ID);
        ID_ = ID;
    }
    
    public Cb_pjsds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Cb_pjsd(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Cb_pjsds(Cb_pjsd parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Cb_pjsds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Cb_pjsds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Cb_pjsd obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Cb_pjsds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
    ...
    public Cb_pjsd getParent() {
        return parentObj_;
    }
    
    public void setParent(Cb_pjsd obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Cb_pjsds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Cb_pjsds.SQLNAME;
    }
