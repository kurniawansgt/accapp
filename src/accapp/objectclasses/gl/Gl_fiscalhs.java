 ...
 *    -  Gl_fiscalhs objs = new Gl_fiscalhs(0); // All Data
 *    or 
 *    - Gl_fiscalhs obj = new Gl_fiscalhs(Gl_fiscalh.PROPERTY_<field>, <value>);
 *      Sample :
 *      Gl_fiscalhs obj = new Gl_fiscalhs(Gl_fiscalh.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Gl_fiscalhs brs = new Gl_fiscalhs(0);

            for (Iterator<Gl_fiscalh> it = brs.iterator(); it.hasNext(); ) {
                
                Gl_fiscalh br = (Gl_fiscalh) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_fiscalh());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Gl_fiscalh br = (Gl_fiscalh) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_fiscalh());
            }
 ...
 * @author windu
 */
public class Gl_fiscalhs  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Gl_fiscalhs";
    public static final String SQLNAME = Gl_fiscalh.TABLE;
    public static String DEFAULTSORT = Gl_fiscalh.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Gl_fiscalh.PROPERTY_ID;
    ...
    private String[][] column = {
        {Gl_fiscalh.PROPERTY_YOP, Gl_fiscalh.PROPERTY_YOP, "200"},
        {Gl_fiscalh.PROPERTY_ACTIVE, Gl_fiscalh.PROPERTY_ACTIVE, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Gl_fiscalhs() throws CodeException {
        this (new Gl_fiscalh(NULLID));
        _isLoaded_ = true;
    }
    
    public Gl_fiscalhs(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_fiscalh(ID);
        ID_ = ID;
    }
    
    public Gl_fiscalhs(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_fiscalh(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Gl_fiscalhs(Gl_fiscalh parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Gl_fiscalhs(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Gl_fiscalhs(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Gl_fiscalh obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Gl_fiscalhs obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Gl_fiscalh) it.next());
        }        
    ...
    public Gl_fiscalh getParent() {
        return parentObj_;
    }
    
    public void setParent(Gl_fiscalh obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Gl_fiscalh obj = (Gl_fiscalh) it.next();
    ...
    public String getDefaultSort() {
        return Gl_fiscalhs.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Gl_fiscalhs.SQLNAME;
    }
    ...
    public void searchData() throws CodeException {
        ...
        String query = " select * from gl_fiscalh order by yop";
