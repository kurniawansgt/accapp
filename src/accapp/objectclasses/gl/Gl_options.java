 ...
 *    -  Gl_options objs = new Gl_options(0); // All Data
 *    or 
 *    - Gl_options obj = new Gl_options(Gl_option.PROPERTY_<field>, <value>);
 *      Sample :
 *      Gl_options obj = new Gl_options(Gl_option.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Gl_options brs = new Gl_options(0);

            for (Iterator<Gl_option> it = brs.iterator(); it.hasNext(); ) {
                
                Gl_option br = (Gl_option) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_option());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Gl_option br = (Gl_option) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_option());
            }
 ...
 * @author windu
 */
public class Gl_options  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Gl_options";
    public static final String SQLNAME = Gl_option.TABLE;
    public static String DEFAULTSORT = Gl_option.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Gl_option.PROPERTY_ID;
    ...
    private String[][] column = {
        {Gl_option.PRIMARYKEY, Gl_option.PRIMARYKEY, "200"}, 
        {Gl_option.PRIMARYKEY, Gl_option.PRIMARYKEY, "200"} 
    };
    ...
    public Gl_options() throws CodeException {
        this (new Gl_option(NULLID));
        _isLoaded_ = true;
    }
    
    public Gl_options(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_option(ID);
        ID_ = ID;
    }
    
    public Gl_options(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_option(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Gl_options(Gl_option parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Gl_options(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Gl_options(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Gl_option obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Gl_options obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Gl_option) it.next());
        }        
    ...
    public Gl_option getParent() {
        return parentObj_;
    }
    
    public void setParent(Gl_option obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Gl_option obj = (Gl_option) it.next();
    ...
    public String getDefaultSort() {
        return Gl_options.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Gl_options.SQLNAME;
    }
