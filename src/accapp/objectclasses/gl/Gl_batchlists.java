 ...
 *    -  Gl_batchlists objs = new Gl_batchlists(0); // All Data
 *    or 
 *    - Gl_batchlists obj = new Gl_batchlists(Gl_batchlist.PROPERTY_<field>, <value>);
 *      Sample :
 *      Gl_batchlists obj = new Gl_batchlists(Gl_batchlist.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Gl_batchlists brs = new Gl_batchlists(0);

            for (Iterator<Gl_batchlist> it = brs.iterator(); it.hasNext(); ) {
                
                Gl_batchlist br = (Gl_batchlist) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_batchlist());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Gl_batchlist br = (Gl_batchlist) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_batchlist());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Gl_batchlist br = (Gl_batchlist) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_batchlist());
            }
 ...
 * @author windu
 */
public class Gl_batchlists  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Gl_batchlists";
    public static final String SQLNAME = Gl_batchlist.TABLE;
    public static String DEFAULTSORT = Gl_batchlist.PROPERTY_BATCHID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Gl_batchlist.PROPERTY_ID;
    ...
    List<Object> list;
    private Gl_batchlist parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Gl_batchlist.PRIMARYKEY, Gl_batchlist.PRIMARYKEY, "200"}, 
        {Gl_batchlist.PROPERTY_BATCHDATE, Gl_batchlist.PROPERTY_BATCHDATE, "200"},
        {Gl_batchlist.PROPERTY_BTCHDESC, Gl_batchlist.PROPERTY_BTCHDESC, "300"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Gl_batchlists() throws CodeException {
        this (new Gl_batchlist(NULLID));
        _isLoaded_ = true;
    }
    
    public Gl_batchlists(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_batchlist(ID);
        ID_ = ID;
    }
    
    public Gl_batchlists(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_batchlist(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Gl_batchlists(Gl_batchlist parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Gl_batchlists(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Gl_batchlists(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Gl_batchlist obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Gl_batchlists obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Gl_batchlist) it.next());
        }        
    ...
    public Gl_batchlist getParent() {
        return parentObj_;
    }
    
    public void setParent(Gl_batchlist obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Gl_batchlist obj = (Gl_batchlist) it.next();
    ...
    public String getDefaultSort() {
        return Gl_batchlists.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Gl_batchlists.SQLNAME;
    }
    ...
    public void searchData(String batchno) throws CodeException {
        ...
        if (batchno.equalsIgnoreCase("")){
            query = " select * from gl_batchlist order by batchid";
        } else {
            query = " select * from gl_batchlist where batchid>='"+batchno+"' order by batchid";
        }
        try {
