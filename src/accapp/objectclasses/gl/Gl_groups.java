 ...
 *    -  Gl_groups objs = new Gl_groups(0); // All Data
 *    or 
 *    - Gl_groups obj = new Gl_groups(Gl_group.PROPERTY_<field>, <value>);
 *      Sample :
 *      Gl_groups obj = new Gl_groups(Gl_group.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Gl_groups brs = new Gl_groups(0);

            for (Iterator<Gl_group> it = brs.iterator(); it.hasNext(); ) {
                
                Gl_group br = (Gl_group) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_group());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Gl_group br = (Gl_group) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_group());
            }
 ...
 * @author windu
 */
public class Gl_groups  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Gl_groups";
    public static final String SQLNAME = Gl_group.TABLE;
    public static String DEFAULTSORT = Gl_group.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Gl_group.PROPERTY_ID;
    ...
    private String[][] column = {
        {Gl_group.PRIMARYKEY, Gl_group.PRIMARYKEY, "200"}, 
        {Gl_group.PROPERTY_ACCGRPDESC, Gl_group.PROPERTY_ACCGRPDESC, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Gl_groups() throws CodeException {
        this (new Gl_group(NULLID));
        _isLoaded_ = true;
    }
    
    public Gl_groups(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_group(ID);
        ID_ = ID;
    }
    
    public Gl_groups(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_group(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Gl_groups(Gl_group parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Gl_groups(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Gl_groups(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Gl_group obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Gl_groups obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Gl_group) it.next());
        }        
    ...
    public Gl_group getParent() {
        return parentObj_;
    }
    
    public void setParent(Gl_group obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public String getDefaultSort() {
        return Gl_groups.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Gl_groups.SQLNAME;
    }
