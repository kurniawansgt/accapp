 ...
 *    -  Gl_accounts objs = new Gl_accounts(0); // All Data
 *    or 
 *    - Gl_accounts obj = new Gl_accounts(Gl_account.PROPERTY_<field>, <value>);
 *      Sample :
 *      Gl_accounts obj = new Gl_accounts(Gl_account.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            for (Iterator<Gl_account> it = brs.iterator(); it.hasNext(); ) {
                
                Gl_account br = (Gl_account) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_account());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Gl_account br = (Gl_account) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_account());
            }
 ...
 * @author windu
 */
public class Gl_accounts  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Gl_accounts";
    public static final String SQLNAME = Gl_account.TABLE;
    public static String DEFAULTSORT = Gl_account.PROPERTY_ACCCODE;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Gl_account.PROPERTY_ID;
    ...
    private String[][] column = {
        {Gl_account.PROPERTY_ACCCODE, Gl_account.PROPERTY_ACCCODE, "120"},
        {Gl_account.PROPERTY_ACCFMTTD, Gl_account.PROPERTY_ACCFMTTD, "130"},
        {Gl_account.PROPERTY_ACCDESC, Gl_account.PROPERTY_ACCDESC, "400"},
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Gl_accounts() throws CodeException {
        this (new Gl_account(NULLID));
        _isLoaded_ = true;
    }
    
    public Gl_accounts(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_account(ID);
        ID_ = ID;
    }
    
    public Gl_accounts(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_account(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Gl_accounts(Gl_account parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Gl_accounts(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Gl_accounts(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Gl_account obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Gl_accounts obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Gl_account) it.next());
        }        
    ...
    public Gl_account getParent() {
        return parentObj_;
    }
    
    public void setParent(Gl_account obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Gl_account obj = (Gl_account) it.next();
    ...
    public String getDefaultSort() {
        return Gl_accounts.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Gl_accounts.SQLNAME;
    }
