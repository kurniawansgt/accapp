 ...
 *    -  Gl_currencyrateds objs = new Gl_currencyrateds(0); // All Data
 *    or 
 *    - Gl_currencyrateds obj = new Gl_currencyrateds(Gl_currencyrated.PROPERTY_<field>, <value>);
 *      Sample :
 *      Gl_currencyrateds obj = new Gl_currencyrateds(Gl_currencyrated.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Gl_currencyrateds brs = new Gl_currencyrateds(0);

            for (Iterator<Gl_currencyrated> it = brs.iterator(); it.hasNext(); ) {
                
                Gl_currencyrated br = (Gl_currencyrated) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_currencyrated());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Gl_currencyrated br = (Gl_currencyrated) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_currencyrated());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Gl_currencyrated br = (Gl_currencyrated) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_currencyrated());
            }
 ...
 * @author windu
 */
public class Gl_currencyrateds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Gl_currencyrateds";
    public static final String SQLNAME = Gl_currencyrated.TABLE;
    public static String DEFAULTSORT = Gl_currencyrated.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Gl_currencyrated.PROPERTY_ID;
    ...
    List<Object> list;
    private Gl_currencyrated parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Gl_currencyrated.PROPERTY_RATEDATE, Gl_currencyrated.PROPERTY_RATEDATE, "200"},
        {Gl_currencyrated.PROPERTY_RATE, Gl_currencyrated.PROPERTY_RATE, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Gl_currencyrateds() throws CodeException {
        this (new Gl_currencyrated(NULLID));
        _isLoaded_ = true;
    }
    
    public Gl_currencyrateds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_currencyrated(ID);
        ID_ = ID;
    }
    
    public Gl_currencyrateds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_currencyrated(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Gl_currencyrateds(Gl_currencyrated parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Gl_currencyrateds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Gl_currencyrateds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Gl_currencyrated(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Gl_currencyrated obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Gl_currencyrateds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Gl_currencyrated) it.next());
        }        
    ...
    public Gl_currencyrated getParent() {
        return parentObj_;
    }
    
    public void setParent(Gl_currencyrated obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Gl_currencyrated obj = (Gl_currencyrated) it.next();
    ...
    public String getDefaultSort() {
        return Gl_currencyrateds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Gl_currencyrateds.SQLNAME;
    }

    public void searchData(String homecur, String ratetype,String sourcecur) throws CodeException {
        ResultSet rs = null;
        DbBean db = DbBean.connect();
        ...
        try {
            if (sourcecur.equalsIgnoreCase("")){
                query = " select * from gl_currencyrated where homecur='" + homecur + "' and ratetype='" + ratetype +"' order by id limit 0,1";

                rs = db.executeQuery(query);
                if (rs.next()){
                    sourcecur=rs.getString("sourcecur");

                    query = " select * from gl_currencyrated where homecur='" + homecur + "' and ratetype='" + ratetype +"' and sourcecur='"+sourcecur+"'";
                    rs = db.executeQuery(query);
                    recordLoad(rs);
                    rs.close();
                }
