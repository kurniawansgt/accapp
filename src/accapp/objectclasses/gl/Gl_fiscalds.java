 ...
 *    -  Gl_fiscalds objs = new Gl_fiscalds(0); // All Data
 *    or 
 *    - Gl_fiscalds obj = new Gl_fiscalds(Gl_fiscald.PROPERTY_<field>, <value>);
 *      Sample :
 *      Gl_fiscalds obj = new Gl_fiscalds(Gl_fiscald.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Gl_fiscalds brs = new Gl_fiscalds(0);

            for (Iterator<Gl_fiscald> it = brs.iterator(); it.hasNext(); ) {
                
                Gl_fiscald br = (Gl_fiscald) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_fiscald());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Gl_fiscald br = (Gl_fiscald) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_fiscald());
            }
 ...
 * @author windu
 */
public class Gl_fiscalds  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Gl_fiscalds";
    public static final String SQLNAME = Gl_fiscald.TABLE;
    public static String DEFAULTSORT = Gl_fiscald.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Gl_fiscald.PROPERTY_ID;
    ...
    private String[][] column = {
        {Gl_fiscald.PRIMARYKEY, Gl_fiscald.PRIMARYKEY, "200"}, 
        {Gl_fiscald.PRIMARYKEY, Gl_fiscald.PRIMARYKEY, "200"} 
    };
    ...
    public Gl_fiscalds() throws CodeException {
        this (new Gl_fiscald(NULLID));
        _isLoaded_ = true;
    }
    
    public Gl_fiscalds(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_fiscald(ID);
        ID_ = ID;
    }
    
    public Gl_fiscalds(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_fiscald(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Gl_fiscalds(Gl_fiscald parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Gl_fiscalds(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Gl_fiscalds(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Gl_fiscald obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Gl_fiscalds obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Gl_fiscald) it.next());
        }        
    ...
    public Gl_fiscald getParent() {
        return parentObj_;
    }
    
    public void setParent(Gl_fiscald obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Gl_fiscald obj = (Gl_fiscald) it.next();
    ...
    public String getDefaultSort() {
        return Gl_fiscalds.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Gl_fiscalds.SQLNAME;
    }
