 ...
 *    -  Gl_segments objs = new Gl_segments(0); // All Data
 *    or 
 *    - Gl_segments obj = new Gl_segments(Gl_segment.PROPERTY_<field>, <value>);
 *      Sample :
 *      Gl_segments obj = new Gl_segments(Gl_segment.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Gl_segments brs = new Gl_segments(0);

            for (Iterator<Gl_segment> it = brs.iterator(); it.hasNext(); ) {
                
                Gl_segment br = (Gl_segment) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_segment());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Gl_segment br = (Gl_segment) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaGl_segment());
            }
 ...
 * @author windu
 */
public class Gl_segments  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Gl_segments";
    public static final String SQLNAME = Gl_segment.TABLE;
    public static String DEFAULTSORT = Gl_segment.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Gl_segment.PROPERTY_ID;
    ...
    private String[][] column = {
        {Gl_segment.PRIMARYKEY, Gl_segment.PRIMARYKEY, "200"}, 
        {Gl_segment.PRIMARYKEY, Gl_segment.PRIMARYKEY, "200"} 
    };
    ...
    public Gl_segments() throws CodeException {
        this (new Gl_segment(NULLID));
        _isLoaded_ = true;
    }
    
    public Gl_segments(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_segment(ID);
        ID_ = ID;
    }
    
    public Gl_segments(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Gl_segment(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Gl_segments(Gl_segment parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Gl_segments(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Gl_segments(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Gl_segment obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Gl_segments obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Gl_segment) it.next());
        }        
    ...
    public Gl_segment getParent() {
        return parentObj_;
    }
    
    public void setParent(Gl_segment obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Gl_segment obj = (Gl_segment) it.next();
    ...
    public String getDefaultSort() {
        return Gl_segments.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Gl_segments.SQLNAME;
    }
