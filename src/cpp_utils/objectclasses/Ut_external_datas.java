 ...
 *    -  Ut_external_datas objs = new Ut_external_datas(0); // All Data
 *    or 
 *    - Ut_external_datas obj = new Ut_external_datas(Ut_external_data.PROPERTY_<field>, <value>);
 *      Sample :
 *      Ut_external_datas obj = new Ut_external_datas(Ut_external_data.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Ut_external_datas brs = new Ut_external_datas(0);

            for (Iterator<Ut_external_data> it = brs.iterator(); it.hasNext(); ) {
                
                Ut_external_data br = (Ut_external_data) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaUt_external_data());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Ut_external_data br = (Ut_external_data) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaUt_external_data());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Ut_external_data br = (Ut_external_data) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaUt_external_data());
            }
 ...
 * @author windu
 */
public class Ut_external_datas  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Ut_external_datas";
    public static final String SQLNAME = Ut_external_data.TABLE;
    public static String DEFAULTSORT = Ut_external_data.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Ut_external_data.PROPERTY_ID;
    ...
    List<Object> list;
    private Ut_external_data parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Ut_external_data.PRIMARYKEY, Ut_external_data.PRIMARYKEY, "200"}, 
        {Ut_external_data.PRIMARYKEY, Ut_external_data.PRIMARYKEY, "200"} 
    };
    ...
    public Ut_external_datas() throws CodeException {
        this (new Ut_external_data(NULLID));
        _isLoaded_ = true;
    }
    
    public Ut_external_datas(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ut_external_data(ID);
        ID_ = ID;
    }
    
    public Ut_external_datas(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Ut_external_data(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Ut_external_datas(Ut_external_data parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Ut_external_datas(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Ut_external_datas(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Ut_external_data(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Ut_external_data obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Ut_external_datas obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Ut_external_data) it.next());
        }        
    ...
    public Ut_external_data getParent() {
        return parentObj_;
    }
    
    public void setParent(Ut_external_data obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Ut_external_data obj = (Ut_external_data) it.next();
    ...
    public String getDefaultSort() {
        return Ut_external_datas.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Ut_external_datas.SQLNAME;
    }
