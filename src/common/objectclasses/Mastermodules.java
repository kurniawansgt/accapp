 ...
 *    -  Mastermodules objs = new Mastermodules(0); // All Data
 *    or 
 *    - Mastermodules obj = new Mastermodules(Mastermodule.PROPERTY_<field>, <value>);
 *      Sample :
 *      Mastermodules obj = new Mastermodules(Mastermodule.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Mastermodules brs = new Mastermodules(0);

            for (Iterator<Mastermodule> it = brs.iterator(); it.hasNext(); ) {
                
                Mastermodule br = (Mastermodule) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMastermodule());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Mastermodule br = (Mastermodule) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMastermodule());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Mastermodule br = (Mastermodule) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMastermodule());
            }
 ...
 * @author windu
 */
public class Mastermodules  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Mastermodules";
    public static final String SQLNAME = Mastermodule.TABLE;
    public static String DEFAULTSORT = Mastermodule.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Mastermodule.PROPERTY_ID;
    ...
    List<Object> list;
    private Mastermodule parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Mastermodule.PROPERTY_MODULE, Mastermodule.PROPERTY_MODULE, "200"}, 
        {Mastermodule.PROPERTY_DESCRIPTION, Mastermodule.PROPERTY_DESCRIPTION, "500"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Mastermodules() throws CodeException {
        this (new Mastermodule(NULLID));
        _isLoaded_ = true;
    }
    
    public Mastermodules(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Mastermodule(ID);
        ID_ = ID;
    }
    
    public Mastermodules(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Mastermodule(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Mastermodules(Mastermodule parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Mastermodules(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Mastermodules(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Mastermodule obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Mastermodules obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Mastermodule) it.next());
        }        
    ...
    public Mastermodule getParent() {
        return parentObj_;
    }
    
    public void setParent(Mastermodule obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Mastermodule obj = (Mastermodule) it.next();
    ...
    public String getDefaultSort() {
        return Mastermodules.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Mastermodules.SQLNAME;
    }
