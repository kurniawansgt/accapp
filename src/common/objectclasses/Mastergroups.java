 ...
 *    -  Mastergroups objs = new Mastergroups(0); // All Data
 *    or 
 *    - Mastergroups obj = new Mastergroups(Mastergroup.PROPERTY_<field>, <value>);
 *      Sample :
 *      Mastergroups obj = new Mastergroups(Mastergroup.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Mastergroups brs = new Mastergroups(0);

            for (Iterator<Mastergroup> it = brs.iterator(); it.hasNext(); ) {
                
                Mastergroup br = (Mastergroup) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMastergroup());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Mastergroup br = (Mastergroup) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMastergroup());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Mastergroup br = (Mastergroup) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMastergroup());
            }
 ...
 * @author windu
 */
public class Mastergroups  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Mastergroups";
    public static final String SQLNAME = Mastergroup.TABLE;
    public static String DEFAULTSORT = Mastergroup.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Mastergroup.PROPERTY_ID;
    ...
    List<Object> list;
    private Mastergroup parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        ...
        {Mastergroup.PROPERTY_KETERANGAN, Mastergroup.PROPERTY_KETERANGAN, "500"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Mastergroups() throws CodeException {
        this (new Mastergroup(NULLID));
        _isLoaded_ = true;
    }
    
    public Mastergroups(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Mastergroup(ID);
        ID_ = ID;
    }
    
    public Mastergroups(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Mastergroup(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Mastergroups(Mastergroup parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Mastergroups(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Mastergroups(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Mastergroup obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Mastergroups obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Mastergroup) it.next());
        }        
    ...
    public void setParent(Mastergroup obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
                Mastergroup obj = (Mastergroup) it.next();
    ...
    public String getDefaultSort() {
        return Mastergroups.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Mastergroups.SQLNAME;
    }
