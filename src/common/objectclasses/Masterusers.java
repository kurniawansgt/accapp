 ...
 *    -  Masterusers objs = new Masterusers(0); // All Data
 *    or 
 *    - Masterusers obj = new Masterusers(Masteruser.PROPERTY_<field>, <value>);
 *      Sample :
 *      Masterusers obj = new Masterusers(Masteruser.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Masterusers brs = new Masterusers(0);

            for (Iterator<Masteruser> it = brs.iterator(); it.hasNext(); ) {
                
                Masteruser br = (Masteruser) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasteruser());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Masteruser br = (Masteruser) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasteruser());
            }
 ...
 * @author windu
 */
public class Masterusers  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Masterusers";
    public static final String SQLNAME = Masteruser.TABLE;
    public static String DEFAULTSORT = Masteruser.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Masteruser.PROPERTY_ID;
    ...
    private String[][] column = {
        {Masteruser.PRIMARYKEY, Masteruser.PRIMARYKEY, "200"}, 
        {Masteruser.PRIMARYKEY, Masteruser.PRIMARYKEY, "200"} 
    };
    ...
    public Masterusers() throws CodeException {
        this (new Masteruser(NULLID));
        _isLoaded_ = true;
    }
    
    public Masterusers(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Masteruser(ID);
        ID_ = ID;
    }
    
    public Masterusers(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Masteruser(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Masterusers(Masteruser parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Masterusers(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Masterusers(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Masteruser obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Masterusers obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Masteruser) it.next());
        }        
    ...
    public Masteruser getParent() {
        return parentObj_;
    }
    
    public void setParent(Masteruser obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Masteruser obj = (Masteruser) it.next();
    ...
    public String getDefaultSort() {
        return Masterusers.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Masterusers.SQLNAME;
    }
