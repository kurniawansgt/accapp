 ...
 *    -  Companys objs = new Companys(0); // All Data
 *    or 
 *    - Companys obj = new Companys(Company.PROPERTY_<field>, <value>);
 *      Sample :
 *      Companys obj = new Companys(Company.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Companys brs = new Companys(0);

            for (Iterator<Company> it = brs.iterator(); it.hasNext(); ) {
                
                Company br = (Company) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCompany());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Company br = (Company) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaCompany());
            }
 ...
 * @author windu
 */
public class Mastercompanys  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Mastercompanys";
    public static final String SQLNAME = Mastercompany.TABLE;
    public static String DEFAULTSORT = Mastercompany.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Mastercompany.PROPERTY_ID;
    ...
    List<Object> list;
    private Mastercompany parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Mastercompany.PRIMARYKEY, Mastercompany.PRIMARYKEY, "200"},
        {Mastercompany.PROPERTY_DESCRIPTION, Mastercompany.PROPERTY_DESCRIPTION, "200"},
        {Mastercompany.PROPERTY_DATABASE, Mastercompany.PROPERTY_DATABASE, "200"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Mastercompanys() throws CodeException {
        this (new Mastercompany(NULLID));
        _isLoaded_ = true;
    }
    
    public Mastercompanys(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Mastercompany(ID);
        ID_ = ID;
    }
    
    public Mastercompanys(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Mastercompany(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Mastercompanys(Mastercompany parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Mastercompanys(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Mastercompanys(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void add(Mastercompany obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Mastercompanys obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Mastercompany) it.next());
        }        
    ...
    public Mastercompany getParent() {
        return parentObj_;
    }
    
    public void setParent(Mastercompany obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Mastercompany obj = (Mastercompany) it.next();
    ...
    public String getDefaultSort() {
        return Mastercompanys.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Mastercompanys.SQLNAME;
    }
