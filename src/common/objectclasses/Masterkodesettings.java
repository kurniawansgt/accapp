 ...
 *    -  Masterkodesettings objs = new Masterkodesettings(0); // All Data
 *    or 
 *    - Masterkodesettings obj = new Masterkodesettings(Masterkodesetting.PROPERTY_<field>, <value>);
 *      Sample :
 *      Masterkodesettings obj = new Masterkodesettings(Masterkodesetting.PROPERTY_ID, "1");
 ...
 * 2. Read Data 
        try {
            ...
            Masterkodesettings brs = new Masterkodesettings(0);

            for (Iterator<Masterkodesetting> it = brs.iterator(); it.hasNext(); ) {
                
                Masterkodesetting br = (Masterkodesetting) it.next();
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterkodesetting());
       ...
       or
        try {
            ...
            for (int i = 0; i < brs.size(); i++) {
                Masterkodesetting br = (Masterkodesetting) brs.list.get(i);
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterkodesetting());
            }
       ...
       try {
            ...
            for (Object Obj : list) {
                Masterkodesetting br = (Masterkodesetting) obj;
                System.out.println( "ID " + br.getID());
                System.out.println( "ID " + br.getNamaMasterkodesetting());
            }
 ...
 * @author windu
 */
public class Masterkodesettings  implements CodeCollectionInt, NavigatorBrowseDataInt{

    public static final long NULLID = 0;
    public static final String CLASSNAME = "Masterkodesettings";
    public static final String SQLNAME = Masterkodesetting.TABLE;
    public static String DEFAULTSORT = Masterkodesetting.PROPERTY_ID;

    public static String SORTASC = "ASC";
    public static String SORTDESC = "DESC";
    
    public static final String FOREIGNKEY = Masterkodesetting.PROPERTY_ID;
    ...
    List<Object> list;
    private Masterkodesetting parentObj_;
    private long ID_;
    ...
    private String[][] column = {
        {Masterkodesetting.PRIMARYKEY, Masterkodesetting.PRIMARYKEY, "200"}, 
        {Masterkodesetting.PROPERTY_DESCRIPTION, Masterkodesetting.PROPERTY_DESCRIPTION, "500"}
    };
    
    
    public String getCLASSNAME(){ return CLASSNAME; }
    
    public Masterkodesettings() throws CodeException {
        this (new Masterkodesetting(NULLID));
        _isLoaded_ = true;
    }
    
    public Masterkodesettings(long ID) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Masterkodesetting(ID);
        ID_ = ID;
    }
    
    public Masterkodesettings(String PrimaryKey) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = new Masterkodesetting(PrimaryKey);
        ID_ = parentObj_.getID();
    }
    
    public Masterkodesettings(Masterkodesetting parent) throws CodeException {
        ClassRegister.instance().register(CLASSNAME);
        parentObj_ = parent;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
    ...
    public Masterkodesettings(String key, String val) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    }
    
    public Masterkodesettings(String key, String val, int searchMethod) {
        ClassRegister.instance().register(CLASSNAME);
        key_ = key;
        val_ = val;
    ...
    public void recordLoad(ResultSet rs) throws CodeException {
           ...
           try {
               while (rs.next()) {
                   list.add(new Masterkodesetting(rs));
               }
               _isLoaded_ = true;
               _isDirty_ = false;
           }catch (SQLException e) {
    ...
    public void add(Masterkodesetting obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        list.add(obj);
    ...
    public void add(Masterkodesettings obj) throws CodeException {
        if (!_isLoaded_) {
            dbRead();
        }
        for (Iterator<Object> it = list.iterator(); it.hasNext(); ) {
            add((Masterkodesetting) it.next());
        }        
    ...
    public Masterkodesetting getParent() {
        return parentObj_;
    }
    
    public void setParent(Masterkodesetting obj) throws CodeException{
        parentObj_ = obj;
        ID_ = parentObj_.getID();
        if (ID_ == NULLID) {
            _isLoaded_ = true;
    ...
    public void fromString(String key, String val) throws CodeException {
        for (Iterator<Object> it = iterator(); it.hasNext(); ) {
                Masterkodesetting obj = (Masterkodesetting) it.next();
    ...
    public String getDefaultSort() {
        return Masterkodesettings.DEFAULTSORT;
    }

    public String getSQLNAME(){
        return Masterkodesettings.SQLNAME;
    }
